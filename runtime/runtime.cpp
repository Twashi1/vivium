...
void _submit(State& state) {
  ...
  state.pipelineComponents =
      state.registry.createView<Owned<PipelineComponent>>();

  for (auto const& viewElement : state.pipelineComponents) {
    PipelineComponent const& pipeline = viewElement.get<PipelineComponent>();

    state.pipelineInstances.push_back(
        _pipelineInstanceFromComponent(state, pipeline, viewElement.entity));
  }
}
...
PipelineInstance _pipelineInstanceFromComponent(
  ...
  BufferComponent const& indexBuffer =
      state.registry.getComponent<BufferComponent>(
          instance.component.indexBuffer);
  BufferComponent const& vertexBuffer =
      state.registry.getComponent<BufferComponent>(
          instance.component.vertexBuffer);
  ShaderComponent const& fragmentShader =
      state.registry.getComponent<ShaderComponent>(
          instance.component.fragmentShader);
  ShaderComponent const& vertexShader =
      state.registry.getComponent<ShaderComponent>(
          instance.component.vertexShader);

  submitResource(
      state.manager, &instance.indexBuffer.reference, MemoryType::DEVICE,
      std::vector<BufferSpecification>(
          {BufferSpecification(indexBuffer.data.size(), indexBuffer.usage)}));

  submitResource(
      state.manager, &instance.vertexBuffer.reference, MemoryType::DEVICE,
      std::vector<BufferSpecification>(
          {BufferSpecification(vertexBuffer.data.size(), vertexBuffer.usage)}));

  ShaderSpecification vertexShaderSpec = compileShader(
      vertexShader.type, vertexShader.filename.c_str(), "out_vert.txt");
  ShaderSpecification fragmentShaderSpec = compileShader(
      fragmentShader.type, fragmentShader.filename.c_str(), "out_frag.txt");

  submitResource(state.manager, &instance.vertexShader.reference,
                 std::vector<ShaderSpecification>({vertexShaderSpec}));
  submitResource(state.manager, &instance.fragmentShader.reference,
                 std::vector<ShaderSpecification>({fragmentShaderSpec}));

  std::vector<UniformData> uniformData;

  DescriptorSetComponent const& descriptorSet =
      state.registry.getComponent<DescriptorSetComponent>(
          instance.component.descriptorSet);
  DescriptorLayoutComponent const& descriptorLayout =
      state.registry.getComponent<DescriptorLayoutComponent>(
          instance.component.descriptorLayout);
  ...
  for (Entity const item : descriptorSet.bindingData) {
    ...
    if (state.registry.hasComponent<BufferComponent>(item)) {
      BufferComponent const& bufferPart =
          state.registry.getComponent<BufferComponent>(item);
      ...
      submitResource(
          state.manager, &object.buffer.reference, MemoryType::UNIFORM,
          std::vector<BufferSpecification>(
              {BufferSpecification(bufferPart.data.size(), bufferPart.usage)}));

      uniformData.push_back(UniformData::fromBuffer(object.buffer.reference,
                                                    bufferPart.data.size(), 0));
    } else if (state.registry.hasComponent<TextureComponent>(item)) {
      TextureComponent const& texturePart =
          state.registry.getComponent<TextureComponent>(item);

      // TODO: Some management/resource system to de-duplicate textures
      submitResource(state.manager, &object.texture.reference,
                     std::vector<TextureSpecification>(
                         {TextureSpecification::fromImageFile(
                             texturePart.filename.c_str(), texturePart.format,
                             texturePart.filter)}));
      ...
      VIVIUM_LOG(LogSeverity::ERROR,
                 "Failed to detect valid descriptor item on entity {}",
  ...
  submitResource(
      state.manager, &instance.layout.reference,
      std::vector<DescriptorLayoutSpecification>(
          {DescriptorLayoutSpecification(descriptorLayout.bindings)}));

  submitResource(
      state.manager, &instance.descriptor.reference,
      std::vector<DescriptorSetSpecification>({DescriptorSetSpecification(
          instance.layout.reference, uniformData)}));

  BufferLayout bufferLayout = BufferLayout::fromTypes(
      state.registry.getComponent<BufferLayoutComponent>(component.bufferLayout)
          .types);

  submitResource(
      state.manager, &instance.pipeline.reference,
      std::vector<PipelineSpecification>({PipelineSpecification::fromWindow(
          std::vector<ShaderReference>({instance.vertexShader.reference,
                                        instance.fragmentShader.reference}),
          bufferLayout,
...
ScriptMetadata _loadScript(State& state, std::string path) {
  ...
  if (luaL_dofile(state.L, path.c_str()) != LUA_OK) {
    VIVIUM_LOG(LogSeverity::ERROR, "Error loading script {}: {}", path,
  ...
  if (!lua_istable(state.L, -1)) {
    VIVIUM_LOG(LogSeverity::ERROR,
               "Script ({}) should return table with functions", path);
...
void _runScriptFunction(State& state, int functionIndex) {
  ...
  if (!lua_isfunction(state.L, -1)) {
    VIVIUM_LOG(LogSeverity::ERROR, "Failed to get function by index {}",
               functionIndex);
    lua_pop(state.L, 1);
  ...
  if (lua_pcall(state.L, 0, 0, 0) != LUA_OK) {
    ...
    VIVIUM_LOG(LogSeverity::ERROR, "Failed to run function: {}",
               lua_tostring(state.L, -1));
    lua_pop(state.L, 1);
    return;
  }
...
void _setup(State& state) {
  ...
  for (PipelineInstance& pipeline : state.pipelineInstances) {
    ...
    for (DescriptorSetObjects& object : pipeline.descriptorObjects) {
      ...
      switch (object.type) {
        ...
        case UniformType::TEXTURE:
          convertResourceReference(state.manager, object.texture);
          break;
        case UniformType::FRAMEBUFFER:
          convertResourceReference(state.manager, object.framebuffer);
          break;
        default:
          VIVIUM_LOG(LogSeverity::ERROR, "Invalid descriptor uniform type");
          break;
      }
    }
    ...
    BufferComponent const& indexBuffer =
        state.registry.getComponent<BufferComponent>(
            pipeline.component.indexBuffer);
    BufferComponent const& vertexBuffer =
        state.registry.getComponent<BufferComponent>(
            pipeline.component.vertexBuffer);
    ...
    _cmdCreateTransientStagingBuffer(
        ...
        vertexBufferSize + indexBufferSize, &stagingMapping);
    ...
    std::memcpy(stagingMapping, vertexBuffer.data.data(), vertexBufferSize);
    cmdTransferBuffer(state.context, resource, vertexBufferSize, 0,
                      pipeline.vertexBuffer.resource);

    std::memcpy(reinterpret_cast<uint8_t*>(stagingMapping) + vertexBufferSize,
                indexBuffer.data.data(), indexBufferSize);
    cmdTransferBuffer(state.context, resource, indexBufferSize,
                      vertexBufferSize, pipeline.indexBuffer.resource);

    contextEndTransfer(state.context, state.engine);

    _cmdFreeTransientStagingBuffer(state.engine, stagingBuffer,
                                   temporaryMemory);
    ...
    state.registry.addComponent<Buffer>(pipeline.component.indexBuffer,
                                        pipeline.indexBuffer.resource);
    state.registry.addComponent<Buffer>(pipeline.component.vertexBuffer,
                                        pipeline.vertexBuffer.resource);
    state.registry.addComponent<DescriptorSet>(pipeline.component.descriptorSet,
                                               pipeline.descriptor.resource);
    state.registry.addComponent<Pipeline>(pipeline.entity,
    ...
    for (DescriptorSetObjects& object : pipeline.descriptorObjects) {
      switch (object.type) {
        ...
        case UniformType::STORAGE_BUFFER:
        case UniformType::DYNAMIC_UNIFORM_BUFFER:
          state.registry.addComponent<Buffer>(object.entity,
                                              object.buffer.resource);
          break;
        case UniformType::TEXTURE:
          state.registry.addComponent<Texture>(object.entity,
                                               object.texture.resource);
          break;
        // TODO: framebuffer might be more difficult to control?
        case UniformType::FRAMEBUFFER:
          state.registry.addComponent<Framebuffer>(object.entity,
                                                   object.framebuffer.resource);
          break;
      }
    }
  }
  ...
  for (PipelineInstance& instance : state.pipelineInstances) {
    for (DescriptorSetObjects& object : instance.descriptorObjects) {
      switch (object.type) {
        ...
        case UniformType::STORAGE_BUFFER:
        case UniformType::DYNAMIC_UNIFORM_BUFFER:
          setBuffer(object.buffer.resource, 0,
                    object.bufferComponent.data.data(),
                    object.bufferComponent.data.size() * sizeof(float));
          break;
        case UniformType::TEXTURE:
        case UniformType::FRAMEBUFFER:
          break;
...
void _loadLuaComponentsEnum(State& state) {
  ...
  lua_pushinteger(state.L,
                  static_cast<uint32_t>(VulkanComponent::BUFFER_LAYOUT));
  lua_setglobal(state.L, "vBUFFER_LAYOUT");

  lua_pushinteger(state.L,
                  static_cast<uint32_t>(VulkanComponent::DESCRIPTOR_LAYOUT));
  ...
  lua_pushinteger(state.L,
                  static_cast<uint32_t>(VulkanComponent::DESCRIPTOR_SET));
...
int _luaBlock(lua_State* L) { return luaL_error(L, "Vivium access denied"); }

int _luaCreateEntity(lua_State* L) {
  LuaContext* context =
      static_cast<LuaContext*>(lua_touserdata(L, lua_upvalueindex(1)));
...
int _luaEntityString(lua_State* L) {
  Entity entity =
      *static_cast<Entity*>(luaL_checkudata(L, 1, ENTITY_TABLE_NAME));
  std::string representation =
      std::format("<Entity: id={}>", static_cast<uint32_t>(entity));
...
int _luaGetComponent(lua_State* L) {
  ...
  VulkanComponent componentType =
      static_cast<VulkanComponent>(luaL_checkinteger(L, 1));

  Entity* entity =
      static_cast<Entity*>(luaL_checkudata(L, 2, ENTITY_TABLE_NAME));
  ...
  switch (componentType) {
    ...
    case VulkanComponent::PIPELINE:
      break;
    default:
      VIVIUM_LOG(LogSeverity::FATAL, "Received invalid component type");
      break;
  }
...
int _luaSetBufferData(lua_State* L) {
  ...
  Entity* entity =
      static_cast<Entity*>(luaL_checkudata(L, 3, ENTITY_TABLE_NAME));
    ...
    if (lua_isnumber(L, -1)) {
      ...
      switch (dataType) {
        case LuaDataType::UINT8:
          *reinterpret_cast<uint8_t*>(elementData) =
              static_cast<uint8_t>(lua_tointeger(L, -1));
          typeSize = sizeof(uint8_t);
          break;
        case LuaDataType::UINT16:
          *reinterpret_cast<uint16_t*>(elementData) =
              static_cast<uint16_t>(lua_tointeger(L, -1));
          typeSize = sizeof(uint16_t);
          break;
        case LuaDataType::UINT32:
          *reinterpret_cast<uint32_t*>(elementData) =
              static_cast<uint32_t>(lua_tointeger(L, -1));
          typeSize = sizeof(uint32_t);
          break;
        case LuaDataType::FLOAT:
          *reinterpret_cast<float*>(elementData) =
              static_cast<float>(lua_tonumber(L, -1));
          typeSize = sizeof(float);
          break;
        case LuaDataType::VEC2:
          break;
        case LuaDataType::VEC3:
          break;
        default:
          break;
      }
      ...
      memcpy(newBufferData.data() + newBufferData.size() - typeSize,
             elementData, typeSize);
    }
  ...
  // TODO: lots of code
  // 1. if we're in the submit phase, just change the buffer data and number of
  // elements (count?)
  // 2. if we're in a phase after, we ensure we have the same number of elements
  // and length of data
  ...
  if (context->stage == Stage::SUBMIT) {
    ...
    BufferComponent& buffer =
        context->registry->getComponent<BufferComponent>(*entity);

    buffer.data = std::move(newBufferData);
  } else if (context->stage == Stage::SETUP ||
             context->stage == Stage::UPDATE || context->stage == Stage::DRAW) {
...
int _luaDrawIndex(lua_State* L) {
  ...
  VIVIUM_ASSERT(context->registry->hasComponent<Pipeline>(*entity),
                "Entity isn't a rendering pipeline");
  ...
  VIVIUM_ASSERT(foundPipeline, "Couldn't find pipeline instance for entity {}",
                *entity);
