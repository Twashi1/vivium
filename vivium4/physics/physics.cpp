...
PenetrationManifold::PenetrationManifold()
    : depth(0.0f),
      vector(0.0f),
      contacts({F32x2(0.0f), F32x2(0.0f)}),
      contactCount(0) {}
...
EdgeManifold axisOfLeastPenetration(const Polygon& a, const Polygon& b,
  ...
  for (uint64_t i = 0; i < a.vertices.size(); i++) {
    ...
    // Get B's support in the direction of the negation of this face of A's
    // normal
    F32x2 support = supportPolygon(b, -bSpaceNormalA);
    // Transform vertex from A's model space into B's model space
    // A -> World -> B
    F32x2 vertex =
        unapplyTransform(applyTransform(a.vertices[i], aTransform), bTransform);
...
std::array<F32x2, 2> getIncidentFace(const Polygon& reference,
  ...
  F32x2 incidentSpaceReferenceNormal =
      incidentTransform.rotationInverse *
      (referenceTransform.rotation * reference.normals[referenceIndex]);
  ...
  faceVertices[0] =
      applyTransform(incident.vertices[minimumIndex], incidentTransform);
  faceVertices[1] = applyTransform(
      incident.vertices[minimumIndex == incident.vertices.size() - 1
                            ? 0
                            : minimumIndex + 1],
      incidentTransform);
...
PenetrationManifold polygonToPolygon(const Polygon& a, const Polygon& b,
  ...
  std::array<F32x2, 2> incidentFace =
      getIncidentFace(*reference, *incident, *referenceTransform,
                      *incidentTransform, referenceManifold->edgeIndex);

  F32x2 referenceFaceVertex0 = applyTransform(
      reference->vertices[referenceManifold->edgeIndex], *referenceTransform);
  F32x2 referenceFaceVertex1 = applyTransform(
      reference->vertices[referenceManifold->edgeIndex ==
                                  reference->vertices.size() - 1
                              ? 0
                              : referenceManifold->edgeIndex + 1],
      *referenceTransform);

  F32x2 referenceFaceVector =
      F32x2::normalise(referenceFaceVertex1 - referenceFaceVertex0);
  F32x2 referenceFaceNormal = F32x2::left(referenceFaceVector);

  float referenceClipped =
      F32x2::dot(referenceFaceNormal, referenceFaceVertex0);
  ...
  if (clip(-referenceFaceVector, negativeVector, incidentFace) < 2)
    return manifold;
  if (clip(referenceFaceVector, positiveVector, incidentFace) < 2)
    return manifold;
  ...
  float separation =
      F32x2::dot(referenceFaceNormal, incidentFace[0]) - referenceClipped;
  ...
  separation =
      F32x2::dot(referenceFaceNormal, incidentFace[1]) - referenceClipped;
...
bool broadCollisionCheck(Body const& a, Body const& b) {
  ...
  return AABBIntersectAABB(
      a.shape.getMin() + a.position, a.shape.getMax() + a.position,
      b.shape.getMin() + b.position, b.shape.getMax() + b.position);
}
...
void checkCollisionAndResolve(Body& a, Body& b) {
  ...
  PenetrationManifold manifold =
      polygonToPolygon(*polyA, *polyB, transformA, transformB);
  ...
  // Bouncy * not bouncy = not bouncy, bouncy * bouncy = very bouncy, not bouncy
  // * not bouncy = not bouncy
  float restitution = a.material.restitution * b.material.restitution;
  // Approximations, bias towards both values being high
  float staticFriction =
      std::sqrt(a.material.staticFriction * b.material.staticFriction);
  float dynamicFriction =
      std::sqrt(a.material.dynamicFriction * b.material.dynamicFriction);
  ...
  for (uint64_t i = 0; i < manifold.contactCount; i++) {
    ...
    F32x2 relativeVelocity =
        b.velocity + b.angularVelocity * F32x2::right(contactB) - a.velocity -
        a.angularVelocity * F32x2::right(contactA);
    ...
    float reactionLength =
        -(1.0f + restitution) * velocityLength * massSum * inverseContactCount;
    ...
    // Project collision normal onto velocity (length), and multiply collision
    // normal by length, then
    //	take the tangent as the norm of the scaled collision normal to the
    // relative velocity from A to B 	This tangent is perpendicular to the
    // normal closest to the relative velocity (so is further from relative
    // velocity)
    F32x2 contactTangent = F32x2::normalise(
        relativeVelocity -
        (manifold.vector * F32x2::dot(relativeVelocity, manifold.vector)));

    // Magnitude of friction impulse in direction of tangent
    // Friction is applied opposite to the tangent vector, hence negation
    float frictionLength = -F32x2::dot(relativeVelocity, contactTangent) *
                           massSum * inverseContactCount;
    ...
    // Coulomb's law
    // Clamp magnitude below some threshold, "if slow moving, do static, if fast
    // moving, do dynamic" If F < mu * R, resolve static friction
  ...
  // Just move each object (position) along the collision normal by a small
  // amount
