...
namespace Vivium {
Registry::Registry() : signatures(Signature{}) {
...
Registry::~Registry() {
  for (ComponentArray* pool : componentPools) {
    if (pool != nullptr && !pool->requiresDeserialise) delete pool;
  }
}
...
void Registry::free(Entity entity) {
  for (ComponentArray* pool : componentPools) {
    ...
    pool->free(entity);
    signatures.index(getIdentifier(entity))
        .set(pool->manager.componentTypeIndex, 0);
  }
...
void Registry::moveEntityIntoOwningGroup(Entity entity,
