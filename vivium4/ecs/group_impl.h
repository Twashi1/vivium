...
template <typename T>
bool GroupMetadata::contains() {
  return affectedComponents.test(registry->_getTypeIndex<T>());
}

template <typename... Ts>
bool GroupMetadata::any() {
  return (contains<Ts>() || ...);
}
template <typename... Ts>
bool GroupMetadata::all() {
  return (contains<Ts>() && ...);
}
...
template <OwnershipTag... WrappedTypes>
View<WrappedTypes...>::ViewIterator::reference
View<WrappedTypes...>::ViewIterator::operator*() {
  return current;
}
template <OwnershipTag... WrappedTypes>
View<WrappedTypes...>::ViewIterator::pointer
View<WrappedTypes...>::ViewIterator::operator->() {
  return &current;
}

template <OwnershipTag... WrappedTypes>
struct View<WrappedTypes...>::ViewIterator&
View<WrappedTypes...>::ViewIterator::operator++() {
  ...
  if (!groupMetadata->ownedComponents.any()) {
    while (groupMetadata->containsSignature(
               registry->signatures.get(getIdentifier(current.entity))) &&
           current.index < groupMetadata->groupSize) {
      current.entity = ownedEntityArray[current.index++];
    }
  }
...
template <OwnershipTag... WrappedTypes>
View<WrappedTypes...>::ViewIterator
View<WrappedTypes...>::ViewIterator::operator++(int) {
...
template <OwnershipTag... WrappedTypes>
bool View<WrappedTypes...>::ViewIterator::operator==(
    ViewIterator const& other) {
...
View<WrappedTypes...>::ViewIterator View<WrappedTypes...>::end() {
  return ViewIterator(registry, ownedEntityArray, groupMetadata,
                      groupMetadata->groupSize, ECS_ENTITY_DEAD);
}
}  // namespace Vivium
