...
struct PagedArray {
  ...
  PagedArray(T const& defaultValue) : PagedArray() {
...
template <SerialiserInterface Interface, typename T, uint64_t pageSize,
          uint64_t capacity>
void serialiseWrite(PagedArray<T, pageSize, capacity> const& pagedArray,
                    Interface& interface) {
...
template <SerialiserInterface Interface, typename T, uint64_t pageSize,
          uint64_t capacity>
void serialiseRead(PagedArray<T, pageSize, capacity>* pagedArray,
                   Interface& interface) {
