...
namespace Vivium {
	...
	{
		vkWaitForFences(engine.device, 1, &framebuffer.inFlightFences[framebuffer.currentFrameIndex], VK_TRUE, UINT64_MAX);
		vkResetFences(engine.device, 1, &framebuffer.inFlightFences[framebuffer.currentFrameIndex]);

		vkResetCommandBuffer(framebuffer.commandBuffers[framebuffer.currentFrameIndex], 0);

		VkCommandBufferBeginInfo beginInfo{};
		beginInfo.sType = VK_STRUCTURE_TYPE_COMMAND_BUFFER_BEGIN_INFO;

		VIVIUM_VK_CHECK(vkBeginCommandBuffer(framebuffer.commandBuffers[framebuffer.currentFrameIndex], &beginInfo),
	...
	void endFramebufferFrame(Framebuffer& framebuffer, CommandContext& context, Engine& engine)
	{
		...
		submitInfo.pCommandBuffers = &framebuffer.commandBuffers[framebuffer.currentFrameIndex];

		VIVIUM_VK_CHECK(vkQueueSubmit(engine.graphicsQueue, 1, &submitInfo, framebuffer.inFlightFences[framebuffer.currentFrameIndex]),
			"Failed to submit draw command to buffer");

		framebuffer.currentFrameIndex = (framebuffer.currentFrameIndex + 1) % VIVIUM_FRAMES_IN_FLIGHT;
	}
