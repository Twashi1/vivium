...
int getRequestedMultisamples(Engine& engine, int multisampleCount) {
  ...
  VkSampleCountFlags availableMultisampleCounts =
      deviceProperties.limits.framebufferColorSampleCounts;

  VIVIUM_ASSERT((multisampleCount != 0) &&
  ...
  for (uint32_t i = 0; i < possibleSampleCounts.size(); i++) {
    ...
    if ((possibleSampleCounts[i] <= multisampleCount) &&
...
void beginFramebufferFrame(Framebuffer& framebuffer, CommandContext& context,
                           Engine& engine) {
  vkWaitForFences(engine.device, 1,
                  &framebuffer.inFlightFences[framebuffer.currentFrameIndex],
                  VK_TRUE, UINT64_MAX);
  vkResetFences(engine.device, 1,
                &framebuffer.inFlightFences[framebuffer.currentFrameIndex]);

  vkResetCommandBuffer(
      framebuffer.commandBuffers[framebuffer.currentFrameIndex], 0);

  VkCommandBufferBeginInfo beginInfo{};
  beginInfo.sType = VK_STRUCTURE_TYPE_COMMAND_BUFFER_BEGIN_INFO;

  VIVIUM_VK_CHECK(vkBeginCommandBuffer(
                      framebuffer.commandBuffers[framebuffer.currentFrameIndex],
                      &beginInfo),
...
void beginFramebufferRender(Framebuffer& framebuffer, CommandContext& context) {
  ...
  renderPassBeginInfo.renderArea.extent.width =
      static_cast<uint32_t>(framebuffer.dimensions.x);
  renderPassBeginInfo.renderArea.extent.height =
      static_cast<uint32_t>(framebuffer.dimensions.y);
  renderPassBeginInfo.clearValueCount = 1;
  renderPassBeginInfo.pClearValues = &clearValue;

  vkCmdBeginRenderPass(context.currentCommandBuffer, &renderPassBeginInfo,
...
void endFramebufferFrame(Framebuffer& framebuffer, CommandContext& context,
                         Engine& engine) {
  ...
  framebuffer.currentFrameIndex =
      (framebuffer.currentFrameIndex + 1) % VIVIUM_FRAMES_IN_FLIGHT;
}
