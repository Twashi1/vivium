...
void setBuffer(Buffer& buffer, uint64_t bufferOffset, const void* data,
  ...
  std::memcpy(reinterpret_cast<uint8_t*>(buffer.mapping) + bufferOffset,
...
void* getBufferMapping(Buffer& buffer) { return buffer.mapping; }

BufferLayout::Element::Element(uint32_t size, uint32_t offset)
    : size(size), offset(offset) {}

BufferLayout BufferLayout::fromTypes(
    const std::span<const ShaderDataType> types) {
  BufferLayout layout;
