...
ResourceManager::DeviceMemoryHandle _allocateDeviceMemory(
  ...
  memoryAllocationInfo.memoryTypeIndex = findMemoryType(
      engine, memoryTypeBits, static_cast<VkMemoryPropertyFlags>(memoryType));

  // TODO: this used to have some debug information attached
  //  but VK_CHECK doesn't take variadic arguments for printing
  VIVIUM_VK_CHECK(vkAllocateMemory(engine.device, &memoryAllocationInfo,
                                   nullptr, &deviceMemoryHandle.memory),
...
void _allocateBuffers(ResourceManager& manager, Engine& engine,
  ...
  switch (memoryType) {
    ...
    default:
      VIVIUM_LOG(LogSeverity::FATAL,
                 "Invalid memory type to allocate buffer to");
      break;
  }

  std::vector<BufferSpecification>& bufferSpecifications =
      *bufferSpecificationsPtr;
  ...
  for (uint64_t i = 0; i < bufferSpecifications.size(); i++) {
    ...
    _cmdCreateBuffer(engine, &resource.buffer, specification.size,
                     specification.usage, &memoryRequirements, nullptr);
    // Calculate offset this buffer should be at in the device memory
    uint64_t resourceOffset =
        nearestMultiple(totalSize, memoryRequirements.alignment);
  ...
  ResourceManager::DeviceMemoryHandle deviceMemoryHandle =
      _allocateDeviceMemory(manager, engine, memoryTypeBits, memoryType,
                            totalSize);
  ...
  for (uint64_t i = 0; i < bufferSpecifications.size(); i++) {
    ...
    vkBindBufferMemory(engine.device, resource.buffer,
                       deviceMemoryHandle.memory, bufferOffsets[i]);

    if (deviceMemoryHandle.mapping != nullptr) {
      resource.mapping =
          reinterpret_cast<uint8_t*>(deviceMemoryHandle.mapping) +
          bufferOffsets[i];
    }
  }
}
...
void _allocateShaders(ResourceManager& manager, Engine& engine) {
  for (uint64_t i = 0; i < manager.shaders.specifications.size(); i++) {
    ShaderSpecification const& specification =
        manager.shaders.specifications[i];
    ...
    shaderCreateInfo.pCode =
        reinterpret_cast<const uint32_t*>(specification.code.c_str());

    VIVIUM_VK_CHECK(vkCreateShaderModule(engine.device, &shaderCreateInfo,
                                         nullptr, &resource.shader),
                    "Failed to create shader module");
  }
}

void _allocateDescriptorLayouts(ResourceManager& manager, Engine& engine) {
  for (uint64_t i = 0; i < manager.descriptorLayouts.specifications.size();
       i++) {
    DescriptorLayoutSpecification const& specification =
        manager.descriptorLayouts.specifications[i];
    DescriptorLayout& resource = manager.descriptorLayouts.resources[i];

    std::vector<VkDescriptorSetLayoutBinding> vulkanBindings(
        specification.bindings.size());
    ...
    for (uint64_t i = 0; i < specification.bindings.size(); i++) {
      ...
      vulkanBinding.descriptorCount =
          1;  // TODO: UBO arrays would require this to be editable (would it?)
    ...
    VIVIUM_VK_CHECK(vkCreateDescriptorSetLayout(engine.device, &createInfo,
                                                nullptr, &resource.layout),
...
void _allocateTextures(ResourceManager& manager, Engine& engine) {
  ...
  // otherwise we need to batch these staging buffers together (which we should
  // do anyway)
  ...
  // Create command pool
  _cmdCreateCommandPool(engine, &commandPool,
                        VK_COMMAND_POOL_CREATE_TRANSIENT_BIT);
  // Create command buffers
  _cmdCreateCommandBuffers(engine, commandPool, commandBuffers.data(),
                           commandBuffers.size());
  ...
  for (uint64_t i = 0; i < manager.textures.specifications.size(); i++) {
    ...
    TextureSpecification const& specification =
        manager.textures.specifications[i];

    _cmdCreateImage(
        engine, &texture.image, specification.width, specification.height,
        specification.imageFormat,
        // TODO: through some method, convert to n samples
        // TODO: cant do it here though, since this image must be a transfer
        // destination maybe not necessary ^^?
        VK_SAMPLE_COUNT_1_BIT, VK_IMAGE_LAYOUT_UNDEFINED,
        VK_IMAGE_USAGE_TRANSFER_DST_BIT | VK_IMAGE_USAGE_SAMPLED_BIT, nullptr);

    VkMemoryRequirements memoryRequirements;

    vkGetImageMemoryRequirements(engine.device, texture.image,
                                 &memoryRequirements);
  ...
  for (uint64_t i = 0; i < commandBuffers.size(); i++) {
    _cmdBeginCommandBuffer(commandBuffers[i],
                           VK_COMMAND_BUFFER_USAGE_ONE_TIME_SUBMIT_BIT);
  }

  // Create VkDeviceMemory
  ResourceManager::DeviceMemoryHandle deviceMemoryHandle =
      _allocateDeviceMemory(manager, engine, memoryTypeBits, MemoryType::DEVICE,
                            totalSize);
  ...
  for (uint64_t i = 0; i < manager.textures.specifications.size(); i++) {
    ...
    VIVIUM_VK_CHECK(vkBindImageMemory(engine.device, texture.image,
                                      deviceMemoryHandle.memory, offsets[i]),
                    "Failed to bind image to memory");

    textureBarriers.push_back({});

    _cmdTransitionImageLayout(
        texture.image, commandBuffers[0], VK_IMAGE_LAYOUT_UNDEFINED,
        VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, VK_PIPELINE_STAGE_TOP_OF_PIPE_BIT,
        VK_PIPELINE_STAGE_TRANSFER_BIT, VK_ACCESS_NONE,
        VK_ACCESS_TRANSFER_WRITE_BIT, &textureBarriers.back());

    VkDeviceMemory stagingMemory;
    VkBuffer stagingBuffer;
    void* stagingMapping;

    _cmdCreateTransientStagingBuffer(engine, &stagingBuffer, &stagingMemory,
    ...
    memcpy(stagingMapping, specification.data.data(),
           specification.data.size());

    textureRegions.push_back({});

    _cmdCopyBufferToImage(stagingBuffer, texture.image, commandBuffers[1],
    ...
    _cmdTransitionImageLayout(
        texture.image, commandBuffers[2], VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL,
  ...
  _cmdEndCommandBuffer(commandBuffers.data(), commandBuffers.size(),
                       engine.graphicsQueue);
  // TODO: experiment with putting this wait until after creating view and
  // sampler?
  vkQueueWaitIdle(engine.graphicsQueue);

  // TODO: maybe these don't need the image to be completely filled out
  // beforehand?
  for (uint64_t i = 0; i < manager.textures.specifications.size(); i++) {
    Texture& texture = manager.textures.resources[i];
    TextureSpecification const& specification =
    ...
    _cmdCreateView(engine, &texture.view, specification.imageFormat,
                   texture.image, nullptr);
    _cmdCreateSampler(engine, &texture.sampler, specification.imageFilter,
                      nullptr);
  }
  ...
  for (uint64_t i = 0; i < textureBuffers.size(); i++) {
    _cmdFreeTransientStagingBuffer(engine, textureBuffers[i],
                                   textureBufferMemories[i]);
  }

  vkFreeCommandBuffers(engine.device, commandPool,
...
void _allocateFramebuffers(ResourceManager& manager, Engine& engine) {
  ...
  for (uint64_t i = 0; i < manager.framebuffers.specifications.size(); i++) {
    FramebufferSpecification const& specification =
        manager.framebuffers.specifications[i];
    Framebuffer& resource = manager.framebuffers.resources[i];

    _cmdCreateImage(
        engine, &resource.image, static_cast<uint32_t>(resource.dimensions.x),
        static_cast<uint32_t>(resource.dimensions.y), specification.format,
        VK_SAMPLE_COUNT_1_BIT, VK_IMAGE_LAYOUT_UNDEFINED,
    ...
    uint64_t resourceOffset =
        nearestMultiple(totalMemoryRequired, requirements.alignment);
  ...
  ResourceManager::DeviceMemoryHandle memory = _allocateDeviceMemory(
      manager, engine, memoryTypeBits, MemoryType::DEVICE, totalMemoryRequired);
  ...
  for (uint64_t i = 0; i < manager.framebuffers.specifications.size(); i++) {
    ...
    VIVIUM_VK_CHECK(vkBindImageMemory(engine.device, resource.image,
                                      memory.memory, imageMemoryLocations[i]),
                    "Failed to bind image memory");

    _cmdCreateView(engine, &resource.view, specification.format, resource.image,
                   nullptr);
    // TODO: customiseable filter
    _cmdCreateSampler(engine, &resource.sampler, TextureFilter::LINEAR,
                      nullptr);
    ...
    //	maybe its better to perform some transitions one-time (if even
    // possible)?
    ...
    VIVIUM_VK_CHECK(vkCreateRenderPass(engine.device, &renderPassInfo, nullptr,
                                       &resource.renderPass),
                    "Failed render pass");
    ...
    VIVIUM_VK_CHECK(vkCreateFramebuffer(engine.device, &framebufferInfo,
                                        nullptr, &resource.framebuffer),
    ...
    VIVIUM_VK_CHECK(vkCreateCommandPool(engine.device, &poolInfo, nullptr,
    ...
    VIVIUM_VK_CHECK(vkAllocateCommandBuffers(engine.device, &allocateInfo,
...
void _allocateDescriptorSets(ResourceManager& manager, Engine& engine) {
...

  {
    ...
    // NOTE: this is slow O(n^2), could use std::unordered_set, but doubt it
    // would be faster
    std::vector<DescriptorLayoutReference> seenLayouts;

    for (uint64_t i = 0; i < manager.descriptorSets.specifications.size();
         i++) {
      DescriptorSetSpecification const& specification =
          manager.descriptorSets.specifications[i];
      ...
      std::vector<UniformBinding> const& bindings =
          manager.descriptorLayouts
              .specifications[specification.layout.referenceIndex]
              .bindings;
      ...
      for (UniformBinding const& binding : bindings) {
        switch (binding.type) {
          ...
          case UniformType::DYNAMIC_UNIFORM_BUFFER:
            ...
            break;
          default:
            VIVIUM_LOG(LogSeverity::FATAL, "Invalid uniform type");
            break;
        }
      }
    }
  ...
  poolInfo.maxSets = static_cast<uint32_t>(
      manager.descriptorSets.specifications.size());  // TODO: should be fine?

  manager.descriptorPools.push_back(VK_NULL_HANDLE);

  VIVIUM_VK_CHECK(vkCreateDescriptorPool(engine.device, &poolInfo, nullptr,
                                         &manager.descriptorPools.back()),
  ...
  std::vector<VkDescriptorSetLayout> layouts =
      std::vector<VkDescriptorSetLayout>(
          manager.descriptorSets.specifications.size());
  std::vector<VkDescriptorSet> sets = std::vector<VkDescriptorSet>(
      manager.descriptorSets.specifications.size());
  ...
  for (uint64_t i = 0; i < manager.descriptorSets.specifications.size(); i++) {
    layouts[i] =
        _getReference(manager, manager.descriptorSets.specifications[i].layout)
            .layout;
  }
  ...
  setAllocateInfo.descriptorSetCount =
      static_cast<uint32_t>(manager.descriptorSets.specifications.size());
  setAllocateInfo.pSetLayouts = layouts.data();

  VIVIUM_VK_CHECK(
      vkAllocateDescriptorSets(engine.device, &setAllocateInfo, sets.data()),
  ...
  for (uint64_t i = 0; i < manager.descriptorSets.specifications.size(); i++) {
    ...
    // Iterate each uniform in specification, and generate a descriptor set
    // write
    for (uint64_t j = 0; j < specification.uniforms.size(); j++) {
      UniformBinding const& binding =
          manager.descriptorLayouts
              .specifications[specification.layout.referenceIndex]
              .bindings[j];
      ...
      switch (binding.type) {
        ...
        case UniformType::STORAGE_BUFFER: {
          ...
          bufferInfo.buffer =
              _getReference(manager, data.bufferData.buffer).buffer;
        ...
        case UniformType::TEXTURE: {
          ...
          Texture const& texture =
              _getReference(manager, data.textureData.texture);
        ...
        case UniformType::FRAMEBUFFER: {
          ...
          Framebuffer const& framebuffer =
              _getReference(manager, data.framebufferData.framebuffer);
...
void _allocatePipelines(ResourceManager& manager, Engine& engine) {
  VIVIUM_LOG(LogSeverity::DEBUG, "Allocating pipelines");

  for (uint64_t i = 0; i < manager.pipelines.specifications.size(); i++) {
    PipelineSpecification const& specification =
        manager.pipelines.specifications[i];
    Pipeline& resource = manager.pipelines.resources[i];

    switch (specification.target) {
      case _RenderTarget::WINDOW:
        resource.renderPass = specification.windowPass;
        break;
      case _RenderTarget::FRAMEBUFFER:
        resource.renderPass =
            _getReference(manager, specification.framebuffer).renderPass;
        break;
      case _RenderTarget::NONE:
    ...
    for (uint32_t i = 0; i < specification.shaders.size(); i++) {
      ...
      shaderStageInfo.stage = static_cast<VkShaderStageFlagBits>(
          manager.shaders.specifications[shaderReference.referenceIndex].stage);
    ...
    std::vector<VkDescriptorSetLayout> descriptorLayouts(
        specification.descriptorLayouts.size());

    for (uint32_t i = 0; i < descriptorLayouts.size(); i++) {
      descriptorLayouts[i] =
          _getReference(manager, specification.descriptorLayouts[i]).layout;
    }
    ...
    VIVIUM_VK_CHECK(vkCreatePipelineLayout(engine.device, &pipelineLayoutInfo,
                                           nullptr, &resource.layout),
                    "Failed to create pipeline layout");

    if (specification.bindPoint == PipelineBindPoint::GRAPHICS) {
      VkPipelineVertexInputStateCreateInfo vertexInputInfo{};
      vertexInputInfo.sType =
          VK_STRUCTURE_TYPE_PIPELINE_VERTEX_INPUT_STATE_CREATE_INFO;
      vertexInputInfo.vertexBindingDescriptionCount = 1;
      vertexInputInfo.vertexAttributeDescriptionCount = static_cast<uint32_t>(
          specification.bufferLayout.attributeDescriptions.size());
      vertexInputInfo.pVertexBindingDescriptions =
          &(specification.bufferLayout.bindingDescription);
      vertexInputInfo.pVertexAttributeDescriptions =
          specification.bufferLayout.attributeDescriptions.data();
      ...
      multisampling.sampleShadingEnable =
          VK_FALSE;  // can allow this to be enabled to have fragments run
                     // per-sample rather than pixel
      ...
      VIVIUM_VK_CHECK(
          vkCreateGraphicsPipelines(engine.device, VK_NULL_HANDLE, 1,
                                    &pipelineInfo, nullptr, &resource.pipeline),
          "Failed to create graphics pipeline");
    } else if (specification.bindPoint == PipelineBindPoint::COMPUTE) {
      // TODO: for compute shaders we want to create a
      // VkComputePipelineCreateInfo API for this? VkComputePipelineCreateInfo
      VkComputePipelineCreateInfo pipelineInfo{};
      pipelineInfo.sType = VK_STRUCTURE_TYPE_COMPUTE_PIPELINE_CREATE_INFO;
      // TODO: very bad, we never specified that a compute pipeline only ever
      // takes a single compute shader
      // TODO: can we really only take one shader?
      pipelineInfo.stage = shaderStages.data()[0];
      pipelineInfo.layout = resource.layout;
      pipelineInfo.basePipelineHandle = VK_NULL_HANDLE;
      pipelineInfo.basePipelineIndex = -1;

      VIVIUM_LOG(LogSeverity::DEBUG, "About to construct compute pipeline");

      VIVIUM_VK_CHECK(
          vkCreateComputePipelines(engine.device, VK_NULL_HANDLE, 1,
                                   &pipelineInfo, nullptr, &resource.pipeline),
          "Failed to create compute pipeline");

    } else {
      VIVIUM_LOG(LogSeverity::FATAL, "Unimplemented pipeline bind point");
      std::unreachable();
    }
  }
}
...
void submitResource(ResourceManager& manager, BufferReference* memory,
  ...
  if ((static_cast<VkMemoryPropertyFlagBits>(memoryType) &
  ...
  } else if ((static_cast<VkMemoryPropertyFlagBits>(memoryType) &
    ...
    VIVIUM_LOG(LogSeverity::FATAL, "Invalid memory type: {}",
               (uint64_t)memoryType);
  }
}

ResourceManager createManager() { return ResourceManager{}; }

void dropManager(ResourceManager& manager, Engine& engine) {
  // Free vulkan memory
  for (ResourceManager::DeviceMemoryHandle deviceMemoryHandle :
       manager.deviceMemoryHandles) {
    if (deviceMemoryHandle.mapping != nullptr)
...
void submitResource(
    ResourceManager& manager, TextureReference* memory,
