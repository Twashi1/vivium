...
uint32_t _findMemoryType(Engine& engine, uint32_t typeFilter,
  ...
  vkGetPhysicalDeviceMemoryProperties(engine.physicalDevice, &memoryProperties);
...
CommandContext createCommandContext(Engine& engine) {
  ...
  _cmdCreateCommandPool(engine, &context.transferPool,
                        VK_COMMAND_POOL_CREATE_RESET_COMMAND_BUFFER_BIT);
  _cmdCreateCommandBuffers(engine, context.transferPool,
                           &context.transferCommandBuffer, 1);

  return context;
}

void dropCommandContext(CommandContext& context, Engine& engine) {
  for (CommandContext::FunctionArray cleanupArray :
       context.perFrameCleanupArrays) {
    for (std::function<void(void)> function : cleanupArray) {
      function();
    }
  }

  vkFreeCommandBuffers(engine.device, context.transferPool, 1,
...
void contextBeginTransfer(CommandContext& context) {
  _cmdBeginCommandBuffer(context.transferCommandBuffer,
...
void _contextFlush(CommandContext& context, Engine& engine) {
  ...
  for (std::function<void(void)> function :
       context.perFrameCleanupArrays[context.frameIndex]) {
    function();
  }
...
void _cmdCreateRenderPass(Engine& engine, VkRenderPass* renderPass,
  ...
  VIVIUM_VK_CHECK(
      vkCreateRenderPass(engine.device, &renderPassInfo, nullptr, renderPass),
...
void _cmdCreateBuffer(Engine& engine, VkBuffer* buffer, uint64_t size,
  ...
  VIVIUM_VK_CHECK(vkCreateBuffer(engine.device, &bufferCreateInfo,
                                 allocationCallbacks, buffer),
...
void _cmdCreateCommandBuffers(Engine& engine, VkCommandPool pool,
  ...
  VIVIUM_VK_CHECK(vkAllocateCommandBuffers(engine.device, &allocateCreateInfo,
                                           commandBuffers),
...
void _cmdBeginCommandBuffer(VkCommandBuffer commandBuffer,
  ...
  VIVIUM_VK_CHECK(vkBeginCommandBuffer(commandBuffer, &beginInfo),
...
void _cmdEndCommandBuffer(VkCommandBuffer* commandBuffers, uint64_t count,
  ...
  for (uint64_t i = 0; i < count; i++) {
    VIVIUM_VK_CHECK(vkEndCommandBuffer(commandBuffers[i]),
...
void _cmdCreateTransientStagingBuffer(Engine& engine, VkBuffer* buffer,
                                      ...
                                      void** mapping) {
  VkMemoryRequirements memoryRequirements;
  _cmdCreateBuffer(engine, buffer, size, BufferUsage::STAGING,
                   &memoryRequirements, nullptr);

  VkMemoryAllocateInfo allocateInfo{};
  allocateInfo.sType = VK_STRUCTURE_TYPE_MEMORY_ALLOCATE_INFO;
  allocateInfo.allocationSize = memoryRequirements.size;
  allocateInfo.memoryTypeIndex =
      findMemoryType(engine, memoryRequirements.memoryTypeBits,
...
void _cmdCreateImage(Engine& engine, VkImage* image, uint32_t width,
  ...
  VIVIUM_VK_CHECK(vkCreateImage(engine.device, &imageCreateInfo,
                                allocationCallbacks, image),
...
void _cmdCreateView(Engine& engine, VkImageView* view, TextureFormat format,
  ...
  VIVIUM_VK_CHECK(vkCreateImageView(engine.device, &viewCreateInfo,
                                    allocationCallbacks, view),
...
void _cmdCreateSampler(Engine& engine, VkSampler* sampler, TextureFilter filter,
  ...
  VIVIUM_VK_CHECK(vkCreateSampler(engine.device, &samplerCreateInfo,
                                  allocationCallbacks, sampler),
...
void _cmdCreatePipeline(
    ...
    const BufferLayout& bufferLayout, VkSampleCountFlagBits sampleCount,
  ...
  for (uint32_t i = 0; i < shaders.size(); i++) {
    ...
    ShaderSpecification const& shaderSpecification =
        manager.shaders.specifications[shaderReference.referenceIndex];
    Shader const& shader = _getReference(manager, shaderReference);

    VkPipelineShaderStageCreateInfo shaderStageInfo{};
    shaderStageInfo.sType = VK_STRUCTURE_TYPE_PIPELINE_SHADER_STAGE_CREATE_INFO;
    shaderStageInfo.stage =
        static_cast<VkShaderStageFlagBits>(shaderSpecification.stage);
  ...
  vertexInputInfo.vertexAttributeDescriptionCount =
      static_cast<uint32_t>(bufferLayout.attributeDescriptions.size());
  vertexInputInfo.pVertexBindingDescriptions =
      &(bufferLayout.bindingDescription);
  vertexInputInfo.pVertexAttributeDescriptions =
      bufferLayout.attributeDescriptions.data();
  ...
  std::vector<VkDescriptorSetLayout> vulkanDescriptorLayouts(
      descriptorLayouts.size());

  for (uint32_t i = 0; i < descriptorLayouts.size(); i++) {
    vulkanDescriptorLayouts[i] =
        _getReference(manager, descriptorLayouts[i]).layout;
  }
  ...
  VIVIUM_VK_CHECK(
      vkCreateGraphicsPipelines(engine.device, VK_NULL_HANDLE, 1, &pipelineInfo,
                                pipelineAllocationCallback, pipeline),
...
void _cmdCopyBufferToImage(VkBuffer imageBuffer, VkImage image,
  ...
  vkCmdCopyBufferToImage(commandBuffer, imageBuffer, image,
...
void cmdTransferBuffer(CommandContext& context, Buffer const& source,
  ...
  vkCmdCopyBuffer(context.inTransfer ? context.transferCommandBuffer
                                     : context.currentCommandBuffer,
                  source.buffer, destination.buffer, 1, copyRegion);
...
void cmdBindVertexBuffer(CommandContext& context, Buffer const& handle) {
  ...
  vkCmdBindVertexBuffers(context.currentCommandBuffer, 0, 1, &handle.buffer,
                         &_literalZero);
}
...
void cmdBindIndexBuffer(CommandContext& context, Buffer const& handle) {
  ...
  vkCmdBindIndexBuffer(context.currentCommandBuffer, handle.buffer, 0,
...
void cmdWritePushConstants(CommandContext& context, const void* data,
                           ...
                           Pipeline const& pipeline) {
  vkCmdPushConstants(context.inTransfer ? context.transferCommandBuffer
                                        : context.currentCommandBuffer,
                     pipeline.layout, static_cast<VkShaderStageFlags>(stage),
