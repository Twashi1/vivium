...
Button submitButton(ResourceManager& manager, GUIContext& guiContext,
                    ButtonSpecification specification) {
  Button button;
  ...
  button.textBatch = submitTextBatch(
      manager, guiContext,
      TextBatchSpecification{64, &guiContext.consolas64,
                             guiContext.consolas64Texture.reference});
  button.text = createText(
      TextSpecification{button.base, "", specification.textColor,
                        calculateTextMetrics("", *button.textBatch.font),
...
void setButtonText(Button& button, Engine& engine, CommandContext& context,
                   GUIContext& guiContext, std::string_view text) {
  // Early exit if no text
  if (text.size() == 0) return;

  setText(button.text, calculateTextMetrics(text, *button.textBatch.font), text,
          button.textColor, button.text.alignment);

  Text* textObjects[] = {&button.text};
  calculateTextBatch(button.textBatch, textObjects, context, guiContext,
                     engine);
}
...
void renderButtons(CommandContext& context, GUIContext& guiContext,
                   Window& window) {
  ...
  setBuffer(guiContext.button.storageBuffer.resource, 0,
            guiContext.button.buttons.data(),
            guiContext.button.buttons.size() * sizeof(_GUIButtonInstanceData));
  ...
  cmdBindDescriptorSet(context, guiContext.button.descriptorSet.resource,
                       guiContext.button.pipeline.resource);
