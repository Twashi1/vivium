...
void _submitGenericGUIContext(GUIContext& guiContext, ResourceManager& manager,
  ...
  submitResource(
      manager, deviceBuffers.data(), MemoryType::DEVICE,
...
void _submitTextGUIContext(GUIContext& guiContext, ResourceManager& manager,
                           Engine& engine, Window& window) {
  submitResource(
      manager, &guiContext.consolas64Texture.reference,
      std::vector<TextureSpecification>({TextureSpecification::fromFont(
          guiContext.consolas64, TextureFormat::MONOCHROME,
  ...
  submitResource(
      manager, &guiContext.text.descriptorLayout.reference,
...
void _submitButtonGUIContext(GUIContext& guiContext, ResourceManager& manager,
                             Engine& engine, Window& window) {
  submitResource(manager, &guiContext.button.storageBuffer.reference,
                 MemoryType::UNIFORM,
                 std::vector<BufferSpecification>({BufferSpecification(
                     guiContext.MAX_BUTTONS * sizeof(_GUIButtonInstanceData),
                     BufferUsage::STORAGE)}));

  submitResource(
      manager, &guiContext.button.descriptorLayout.reference,
      std::vector<DescriptorLayoutSpecification>({DescriptorLayoutSpecification(
          std::vector<UniformBinding>({UniformBinding(
              ShaderStage::VERTEX, 0, UniformType::STORAGE_BUFFER)}))}));

  submitResource(
      manager, &guiContext.button.descriptorSet.reference,
      std::vector<DescriptorSetSpecification>({DescriptorSetSpecification(
          guiContext.button.descriptorLayout.reference,
          std::vector<UniformData>({UniformData::fromBuffer(
              guiContext.button.storageBuffer.reference,
              guiContext.MAX_BUTTONS * sizeof(_GUIButtonInstanceData), 0)}))}));

  submitResource(manager, &guiContext.button.fragmentShader.reference,
                 std::vector<ShaderSpecification>({compileShader(
                     ShaderStage::FRAGMENT, "vivium4/res/button.frag",
                     "vivium4/res/button_frag.spv")}));

  submitResource(manager, &guiContext.button.vertexShader.reference,
                 std::vector<ShaderSpecification>({compileShader(
                     ShaderStage::VERTEX, "vivium4/res/button.vert",
                     "vivium4/res/button_vert.spv")}));

  submitResource(
      manager, &guiContext.button.pipeline.reference,
      std::vector<PipelineSpecification>({PipelineSpecification::fromWindow(
          std::vector<ShaderReference>(
              {guiContext.button.fragmentShader.reference,
               guiContext.button.vertexShader.reference}),
...
void _submitPanelGUIContext(GUIContext& guiContext, ResourceManager& manager,
                            Engine& engine, Window& window) {
  submitResource(manager, &guiContext.panel.storageBuffer.reference,
                 MemoryType::UNIFORM,
                 std::vector<BufferSpecification>({BufferSpecification(
                     guiContext.MAX_PANELS * sizeof(_GUIPanelInstanceData),
                     BufferUsage::STORAGE)}));

  submitResource(
      manager, &guiContext.panel.descriptorLayout.reference,
      std::vector<DescriptorLayoutSpecification>({DescriptorLayoutSpecification(
          std::vector<UniformBinding>({UniformBinding(
              ShaderStage::VERTEX, 0, UniformType::STORAGE_BUFFER)}))}));

  submitResource(
      manager, &guiContext.panel.descriptorSet.reference,
      std::vector<DescriptorSetSpecification>({DescriptorSetSpecification(
          guiContext.panel.descriptorLayout.reference,
          std::vector<UniformData>({UniformData::fromBuffer(
              guiContext.panel.storageBuffer.reference,
              guiContext.MAX_PANELS * sizeof(_GUIPanelInstanceData), 0)}))}));

  submitResource(manager, &guiContext.panel.fragmentShader.reference,
                 std::vector<ShaderSpecification>({compileShader(
                     ShaderStage::FRAGMENT, "vivium4/res/panel.frag",
                     "vivium4/res/panel_frag.spv")}));

  submitResource(manager, &guiContext.panel.vertexShader.reference,
                 std::vector<ShaderSpecification>({compileShader(
                     ShaderStage::VERTEX, "vivium4/res/panel.vert",
                     "vivium4/res/panel_vert.spv")}));

  submitResource(
      manager, &guiContext.panel.pipeline.reference,
      std::vector<PipelineSpecification>({PipelineSpecification::fromWindow(
          std::vector<ShaderReference>(
              {guiContext.panel.fragmentShader.reference,
               guiContext.panel.vertexShader.reference}),
          BufferLayout::fromTypes(
              std::vector<ShaderDataType>({ShaderDataType::VEC2})),
          std::vector<DescriptorLayoutReference>(
              {guiContext.panel.descriptorLayout.reference}),
...
void _submitSliderGUIContext(GUIContext& guiContext, ResourceManager& manager,
                             Engine& engine, Window& window) {
  submitResource(manager, &guiContext.slider.storageBuffer.reference,
                 MemoryType::UNIFORM,
                 std::vector<BufferSpecification>({BufferSpecification(
                     guiContext.MAX_SLIDERS * sizeof(_GUISliderInstanceData),
                     BufferUsage::STORAGE)}));

  submitResource(
      manager, &guiContext.slider.descriptorLayout.reference,
      std::vector<DescriptorLayoutSpecification>({DescriptorLayoutSpecification(
          std::vector<UniformBinding>({UniformBinding(
              ShaderStage::VERTEX, 0, UniformType::STORAGE_BUFFER)}))}));

  submitResource(
      manager, &guiContext.slider.descriptorSet.reference,
      std::vector<DescriptorSetSpecification>({DescriptorSetSpecification(
          guiContext.slider.descriptorLayout.reference,
          std::vector<UniformData>({UniformData::fromBuffer(
              guiContext.slider.storageBuffer.reference,
              guiContext.MAX_SLIDERS * sizeof(_GUISliderInstanceData), 0)}))}));

  submitResource(manager, &guiContext.slider.fragmentShader.reference,
                 std::vector<ShaderSpecification>({compileShader(
                     ShaderStage::FRAGMENT, "vivium4/res/slider.frag",
                     "vivium4/res/slider_frag.spv")}));

  submitResource(manager, &guiContext.slider.vertexShader.reference,
                 std::vector<ShaderSpecification>({compileShader(
                     ShaderStage::VERTEX, "vivium4/res/slider.vert",
                     "vivium4/res/slider_vert.spv")}));

  submitResource(
      manager, &guiContext.slider.pipeline.reference,
      std::vector<PipelineSpecification>({PipelineSpecification::fromWindow(
          std::vector<ShaderReference>(
              {guiContext.slider.fragmentShader.reference,
               guiContext.slider.vertexShader.reference}),
          BufferLayout::fromTypes(
              std::vector<ShaderDataType>({ShaderDataType::VEC2})),
          std::vector<DescriptorLayoutReference>(
              {guiContext.slider.descriptorLayout.reference}),
...
void _submitSpriteGUIContext(GUIContext& guiContext, ResourceManager& manager,
  ...
  submitResource(manager, &guiContext.sprite.storageBuffer.reference,
                 MemoryType::UNIFORM,
                 std::vector<BufferSpecification>({BufferSpecification(
                     guiContext.MAX_SPRITES * sizeof(_GUISpriteInstanceData),
                     BufferUsage::STORAGE)}));

  submitResource(
      manager, &guiContext.sprite.descriptorLayout.reference,
  ...
  submitResource(
      manager, &guiContext.sprite.texture.reference,
      std::vector<TextureSpecification>({TextureSpecification::fromData(
          guiContext.sprite.atlas->data, guiContext.sprite.atlas->size,
          guiContext.sprite.atlas->format, TextureFilter::NEAREST)}));

  submitResource(
      manager, &guiContext.sprite.descriptorSet.reference,
      std::vector<DescriptorSetSpecification>({DescriptorSetSpecification(
          guiContext.sprite.descriptorLayout.reference,
          std::vector<UniformData>(
              {UniformData::fromBuffer(
                   guiContext.sprite.storageBuffer.reference,
                   guiContext.MAX_SPRITES * sizeof(_GUISpriteInstanceData), 0),
               UniformData::fromTexture(
                   guiContext.sprite.texture.reference)}))}));

  submitResource(manager, &guiContext.sprite.fragmentShader.reference,
                 std::vector<ShaderSpecification>({compileShader(
                     ShaderStage::FRAGMENT, "vivium4/res/sprite.frag",
                     "vivium4/res/sprite_frag.spv")}));

  submitResource(manager, &guiContext.sprite.vertexShader.reference,
                 std::vector<ShaderSpecification>({compileShader(
                     ShaderStage::VERTEX, "vivium4/res/sprite.vert",
                     "vivium4/res/sprite_vert.spv")}));

  submitResource(
      manager, &guiContext.sprite.pipeline.reference,
      std::vector<PipelineSpecification>({PipelineSpecification::fromWindow(
          std::vector<ShaderReference>(
              {guiContext.sprite.fragmentShader.reference,
               guiContext.sprite.vertexShader.reference}),
          BufferLayout::fromTypes(
              std::vector<ShaderDataType>({ShaderDataType::VEC2})),
          std::vector<DescriptorLayoutReference>(
              {guiContext.sprite.descriptorLayout.reference}),
...
void _submitDebugRectGUIContext(GUIContext& guiContext,
                                ...
                                Window& window) {
  submitResource(
      manager, &guiContext.debugRect.storageBuffer.reference,
      MemoryType::UNIFORM,
      std::vector<BufferSpecification>({BufferSpecification(
          guiContext.MAX_DEBUG_RECTS * sizeof(_GUIDebugRectInstanceData),
          BufferUsage::STORAGE)}));

  submitResource(
      manager, &guiContext.debugRect.descriptorLayout.reference,
      std::vector<DescriptorLayoutSpecification>({DescriptorLayoutSpecification(
          std::vector<UniformBinding>({UniformBinding(
              ShaderStage::VERTEX, 0, UniformType::STORAGE_BUFFER)}))}));

  submitResource(
      manager, &guiContext.debugRect.descriptorSet.reference,
      std::vector<DescriptorSetSpecification>({DescriptorSetSpecification(
          guiContext.debugRect.descriptorLayout.reference,
          std::vector<UniformData>({UniformData::fromBuffer(
              guiContext.debugRect.storageBuffer.reference,
              guiContext.MAX_DEBUG_RECTS * sizeof(_GUIDebugRectInstanceData),
              0)}))}));

  submitResource(manager, &guiContext.debugRect.fragmentShader.reference,
                 std::vector<ShaderSpecification>({compileShader(
                     ShaderStage::FRAGMENT, "vivium4/res/debugrect.frag",
                     "vivium4/res/debugrect_frag.spv")}));

  submitResource(manager, &guiContext.debugRect.vertexShader.reference,
                 std::vector<ShaderSpecification>({compileShader(
                     ShaderStage::VERTEX, "vivium4/res/debugrect.vert",
                     "vivium4/res/debugrect_vert.spv")}));

  submitResource(
      manager, &guiContext.debugRect.pipeline.reference,
      std::vector<PipelineSpecification>({PipelineSpecification::fromWindow(
          std::vector<ShaderReference>(
              {guiContext.debugRect.fragmentShader.reference,
               guiContext.debugRect.vertexShader.reference}),
          BufferLayout::fromTypes(
              std::vector<ShaderDataType>({ShaderDataType::VEC2})),
          std::vector<DescriptorLayoutReference>(
              {guiContext.debugRect.descriptorLayout.reference}),
...
GUIElementReference nullGUIParent() { return GUIElementReference(UINT64_MAX); }

// TODO: create doesn't match the pattern, elements that require a setup, should
// also be `submit`
...
void renderGUI(CommandContext& context, GUIContext& guiContext,
               Window& window) {
...
void setupGUIContext(GUIContext& guiContext, ResourceManager& manager,
  ...
  // TODO: big error here, we schedule two moves from the same staging buffer,
  // resulting in an overwrite that gives bogus data!
  // TODO: this is a really old TODO? circumvented this (poorly)
  //	by making a large staging buffer, and using the first n bytes for vertex
  // data
  ...
  _cmdCreateTransientStagingBuffer(engine, &stagingBuffer, &temporaryMemory,
                                   8 * sizeof(float) + 6 * sizeof(uint16_t),
                                   &stagingMapping);
  ...
  cmdTransferBuffer(context, resource, 8 * sizeof(float), 0,
                    guiContext.rectVertexBuffer.resource);

  memcpy(reinterpret_cast<uint8_t*>(stagingMapping) + 8 * sizeof(float),
         indexData, 6 * sizeof(uint16_t));
  cmdTransferBuffer(context, resource, 6 * sizeof(uint16_t), 8 * sizeof(float),
                    guiContext.rectIndexBuffer.resource);
...
void updateGUIContext(GUIContext& guiContext, F32x2 windowDimensions) {
  ...
  updateGUIElement(defaultGUIParent(guiContext), defaultGUIParent(guiContext),
