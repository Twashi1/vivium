...
TextMetrics calculateTextMetrics(std::string_view const& text,
  ...
  for (uint64_t i = 0; i < text.size(); i++) {
    ...
    if (currentBelowLine > belowLineSize) belowLineSize = currentBelowLine;
  ...
  metrics.lineWidths.push_back(currentLineWidth);
  metrics.totalHeight =
      font.fontSize * metrics.newLineCount + metrics.firstLineHeight;
...
std::vector<PerGlyphData> generateTextRenderData(TextMetrics const& metrics,
  ...
  for (uint64_t i = 0; i < text.size(); i++) {
    ...
    // TODO: warn on characters we don't know how to draw, or that should never
    // be drawn
    FontCharacter fontCharacter = font.characters[character];

    if (!isspace(character)) {
      F32x2 bottomLeft =
          F32x2(position.x + fontCharacter.bearing.x * scale.x,
                position.y -
                    (fontCharacter.size.y - fontCharacter.bearing.y) * scale.y);
      F32x2 topRight =
          bottomLeft + F32x2(static_cast<float>(fontCharacter.size.x),
                             static_cast<float>(fontCharacter.size.y)) *
                           scale;

      // TODO: constructor
      renderData.push_back(PerGlyphData{
          bottomLeft, topRight, F32x2(fontCharacter.left, fontCharacter.bottom),
...
void renderTextBatch(CommandContext& context, GUIContext& guiContext,
  ...
  for (TextBatch* batch : guiContext.text.texts) {
    if (indexCountBatch(batch->batch) == 0) {
      continue;
    }

    cmdWritePushConstants(context, &perspective, sizeof(Perspective), 0,
                          ShaderStage::VERTEX,
                          guiContext.text.pipeline.resource);

    cmdBindPipeline(context, guiContext.text.pipeline.resource);
    cmdBindDescriptorSet(context, batch->descriptorSet.resource,
                         guiContext.text.pipeline.resource);
...
void renderTextBatch(TextBatch& text, CommandContext& context,
  ...
  if (indexCountBatch(text.batch) == 0) {
    return;
  }

  cmdWritePushConstants(context, &perspective, sizeof(Perspective), 0,
                        ShaderStage::VERTEX, guiContext.text.pipeline.resource);

  cmdBindPipeline(context, guiContext.text.pipeline.resource);
  cmdBindDescriptorSet(context, text.descriptorSet.resource,
                       guiContext.text.pipeline.resource);
...
void calculateTextBatch(TextBatch& textBatch, std::span<Text*> textObjects,
  ...
  for (Text* text : textObjects) {
    ...
    F32x2 axisScale =
        props.trueDimensions /
        F32x2(text->metrics.maxLineWidth, text->metrics.totalHeightAndBottom);
    ...
    switch (text->alignment) {
      ...
      case TextAlignment::CENTER:
        scaleOrigin = F32x2(0.0f);
        break;
      case TextAlignment::RIGHT:
        VIVIUM_LOG(LogSeverity::FATAL, "Right alignment not implemented");
        break;
      default:
        VIVIUM_LOG(LogSeverity::FATAL, "Invalid alignment");
        break;
    }

    std::vector<PerGlyphData> renderData =
        generateTextRenderData(text->metrics, text->characters, *textBatch.font,
                               F32x2(1.0f), text->alignment);
    ...
    for (PerGlyphData const& glyph : renderData) {
      F32x2 bottomLeft =
          (glyph.bottomLeft - scaleOrigin) * scale + scaleOrigin + translation;
      F32x2 topRight =
          (glyph.topRight - scaleOrigin) * scale + scaleOrigin + translation;

      submitRectangleBatch(textBatch.batch, 0, bottomLeft.x, bottomLeft.y,
                           topRight.x, topRight.y);
      submitRectangleBatch(textBatch.batch, 1, glyph.texBottomLeft.x,
                           glyph.texBottomLeft.y, glyph.texTopRight.x,
                           glyph.texTopRight.y);
      submitElementBatch(
          textBatch.batch, 2,
          {reinterpret_cast<uint8_t*>(textColorData), sizeof(Color) * 4});
...
TextBatch submitTextBatch(ResourceManager& manager, GUIContext& guiContext,
  ...
  text.batch = submitBatch(
      manager, BatchSpecification(specification.maxCharacterCount * 4,
  ...
  VIVIUM_ASSERT(specification.font != nullptr,
                "Must pass font to text batch specification");
  ...
  // We weren't provided the texture, so make it ourselves
    ...
    submitResource(
        manager, &text.fontTexture.reference,
        std::vector<TextureSpecification>({TextureSpecification::fromFont(
            *specification.font, TextureFormat::MONOCHROME,
  ...
  submitResource(
      manager, &text.descriptorSet.reference,
...
Text createText(TextSpecification const& specification,
                GUIContext& guiContext) {
  GUIElementReference base = createGUIElement(guiContext);
  addChild(specification.parent, {&base, 1}, guiContext);

  return Text{base, specification.characters, specification.color,
              specification.metrics, specification.alignment};
}
