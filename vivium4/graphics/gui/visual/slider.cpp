...
void updateSlider(Slider& slider, GUIContext& guiContext) {
  ...
  if (pointInElement(cursorPos, properties(slider, guiContext)) &&
      Input::get(Input::BTN_1)) {
    // Calculate x percentage relative to true position and dimension
    float percent =
        (cursorPos.x - properties(slider, guiContext).truePosition.x) /
        properties(slider, guiContext).trueDimensions.x;
...
void renderSliders(CommandContext& context, GUIContext& guiContext,
                   Window& window) {
  Perspective perspective = orthogonalPerspective2D(windowDimensions(window),
                                                    F32x2(0.0f), 0.0f, 1.0f);

  setBuffer(guiContext.slider.storageBuffer.resource, 0,
            guiContext.slider.sliders.data(),
            guiContext.slider.sliders.size() * sizeof(_GUISliderInstanceData));
  ...
  cmdBindDescriptorSet(context, guiContext.slider.descriptorSet.resource,
                       guiContext.slider.pipeline.resource);
