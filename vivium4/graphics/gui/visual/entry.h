...
struct ObjectEntry {
  ...
  // TODO: note we'd have to re-create this with new buttons if we change the
  // option set
...
template <FiniteObjectType T>
ObjectEntry<T> submitEntry(
    EntrySpecification<ObjectEntry<T>> const& specification,
    GUIContext& context, ResourceManager& resourceManager) {
  ObjectEntry<T> entry;
  ...
  entry.objectView = submitButton(
      resourceManager, context,
      ButtonSpecification(nullGUIParent(), Color(0.25f, 0.25f, 0.25f),
                          Color(0.0f, 0.0f, 0.0f)));
  entry.base = entry.objectView.base;

  entry.dropDownContainer = createContainer(
      context,
      ContainerSpecification(entry.objectView.base, ContainerOrdering::VERTICAL,
                             OffsetMethod::EXTENT));
  ...
  for (T const& value_type : entry.options) {
    ...
    dropDownOption = submitButton(
        resourceManager, context,
        ButtonSpecification(entry.dropDownContainer.base,
...
template <FiniteObjectType T>
void setupEntry(ObjectEntry<T>& entry, ResourceManager& manager, Engine& engine,
  ...
  for (uint64_t i = 0; i < entry.options.size(); i++) {
    ...
    setButtonText(entry.dropDownOptions[i], engine, context, guiContext,
                  getString(entry.options[i]));
  }

  setButtonText(entry.objectView, engine, context, guiContext,
                getString(entry.defaultValue));
...
template <FiniteObjectType T>
void updateEntry(ObjectEntry<T>& entry, GUIContext& guiContext, Engine& engine,
                 CommandContext& context) {
  setButtonText(entry.objectView, engine, context, guiContext,
                getString(entry.currentlySelected));
  ...
  // If we click any button on the drop down menu we change the text of the
  // objectView
  bool clicked = Input::get(Input::BTN_1).state == Input::RELEASE;
  bool hoveringObjectView = pointInElement(
      Input::getCursor(), properties(entry.objectView.base, guiContext));
  bool hoveringDropDown = pointInExtent(
      Input::getCursor(), properties(entry.dropDownContainer.base, guiContext));

  setAsleep(entry.dropDownContainer.base, guiContext, !entry.dropDownOpen);

  // If the drop down is open but our cursor isn't on it, or the object view,
  // close it
  ...
  if (entry.dropDownOpen && hoveringDropDown && clicked) {
    ...
    for (uint64_t i = 0; i < entry.options.size(); i++) {
      ...
      bool hoveringButton = pointInElement(Input::getCursor(),
                                           properties(button.base, guiContext));
...
template <FiniteObjectType T>
void submitEntries(std::span<ObjectEntry<T>*> const entries,
                   GUIContext& context) {
...
ObjectEntry<ObjectType>::ValueType getValue(
    ObjectEntry<ObjectType> const& entry) {
  return entry.currentlySelected;
}

template <FiniteObjectType ObjectType>
void loadValue(ObjectEntry<ObjectType>& entry,
               typename ObjectEntry<ObjectType>::ValueType const& value,
               GUIContext& guiContext) {
  entry.currentlySelected = value;
}

template <FiniteObjectType T>
UploadEntry<T> submitEntry(
    EntrySpecification<UploadEntry<T>> const& specification,
    GUIContext& context, ResourceManager& resourceManager) {
  UploadEntry<T> entry;

  entry.valuePanel = createPanel(
      context, PanelSpecification(nullGUIParent(), Color(0.25f, 0.25f, 0.25f),
                                  Color(0.1f, 0.1f, 0.1f), 0.01f));
  entry.base = entry.valuePanel.base;
  entry.valueButton = submitButton(
      resourceManager, context,
      ButtonSpecification(entry.valuePanel.base, Color(0.25f, 0.25f, 0.25f),
                          Color(0.0f, 0.0f, 0.0f)));

  entry.clearPanel =
      createPanel(context, PanelSpecification(entry.valuePanel.base,
  ...
  VIVIUM_ASSERT(entry.heldItemPointer != nullptr,
                "No pointer to where held item is");

  return entry;
}

template <FiniteObjectType T>
void setupEntry(UploadEntry<T>& entry, ResourceManager& manager, Engine& engine,
                CommandContext& context, GUIContext& guiContext) {
...
template <FiniteObjectType T>
void updateEntry(UploadEntry<T>& entry, GUIContext& guiContext, Engine& engine,
                 CommandContext& context) {
  ...
  bool clicked = Input::get(Input::BTN_1).state == Input::RELEASE;
  bool hoveringUpload = pointInElement(
      Input::getCursor(), properties(entry.valueButton.base, guiContext));
  bool hoveringClear = pointInElement(
      Input::getCursor(), properties(entry.clearPanel.base, guiContext));
...
template <FiniteObjectType T>
void submitEntries(std::span<UploadEntry<T>*> const entries,
                   GUIContext& context) {
...
template <FiniteObjectType ObjectType>
UploadEntry<ObjectType>::ValueType getValue(
    UploadEntry<ObjectType> const& entry) {
  // TODO: bit bad, only actually release selected value if there is one
  // (hasValue)
  VIVIUM_ASSERT(entry.hasValue,
                "Entry didn't have value but we tried to request it");
  return entry.currentlySelected;
}

template <FiniteObjectType ObjectType>
void loadValue(UploadEntry<ObjectType>& entry,
               typename UploadEntry<ObjectType>::ValueType const& value,
               GUIContext& guiContext) {
  entry.hasValue = true;
  entry.currentlySelected = value;
}
...
template <typename ValueEntry>
ListEntry<ValueEntry> submitEntry(
    EntrySpecification<ListEntry<ValueEntry>> const& specification,
    GUIContext& context, ResourceManager& resourceManager) {
  ...
  entry.addEntry = submitButton(
      resourceManager, context,
      ButtonSpecification(entry.entryContainer.base, Color(0.25f, 0.65f, 0.25f),
                          Color(0.0f, 0.0f, 0.0f)));
  entry.numEntries = 0;
  ...
  for (uint64_t i = 0; i < specification.maxEntries; i++) {
    entry.entries[i] = submitEntry(*specification.valueSpecification, context,
                                   resourceManager);
    entry.entryWrapper[i] = createPanel(
        context, PanelSpecification(entry.entryContainer.base,
                                    Color(0.25f, 0.25f, 0.25f),
                                    Color(0.0f, 0.0f, 0.0f), 0.01f));

    entry.deleteEntry[i] = createPanel(
        context,
        PanelSpecification(entry.entryWrapper[i].base, Color(0.6f, 0.1f, 0.1f),
                           Color(0.0f, 0.0f, 0.0f), 0.01f));
    entry.entryUp[i] = createPanel(
        context,
        PanelSpecification(entry.entryWrapper[i].base, Color(0.6f, 0.6f, 0.6f),
                           Color(0.0f, 0.0f, 0.0f), 0.01f));
    entry.entryDown[i] = createPanel(
        context,
        PanelSpecification(entry.entryWrapper[i].base, Color(0.6f, 0.6f, 0.6f),
                           Color(0.0f, 0.0f, 0.0f), 0.01f));

    properties(entry.entryWrapper[i].base, context).dimensions =
        F32x2(1.0f, 1.0f);
    ...
    properties(entry.deleteEntry[i].base, context).dimensions =
        F32x2(0.1f, 0.5f);
...
template <typename ValueEntry>
void setupEntry(ListEntry<ValueEntry>& entry, ResourceManager& resourceManager,
                Engine& engine, CommandContext& context,
                GUIContext& guiContext) {
...
template <typename ValueEntry>
void _removeIndexFromListEntry(ListEntry<ValueEntry>& entry,
                               GUIContext& guiContext, uint64_t index) {
  ...
  std::rotate(entry.entries.begin() + index, entry.entries.begin() + index + 1,
              entry.entries.end());
  std::rotate(entry.entryWrapper.begin() + index,
              entry.entryWrapper.begin() + index + 1, entry.entryWrapper.end());
  std::rotate(entry.deleteEntry.begin() + index,
              entry.deleteEntry.begin() + index + 1, entry.deleteEntry.end());
  std::rotate(entry.entryUp.begin() + index, entry.entryUp.begin() + index + 1,
              entry.entryUp.end());
  std::rotate(entry.entryDown.begin() + index,
              entry.entryDown.begin() + index + 1, entry.entryDown.end());
  // TODO: terrible code
  // NOTE: add 1 is because add entry is also stored in the container
  rotateChild(entry.entryContainer.base, index + 1, index + 2,
              getChildren(entry.entryContainer.base, guiContext).size(),
              guiContext);
...
template <typename ValueEntry>
void _insertIndexToListEntry(ListEntry<ValueEntry>& entry,
                             GUIContext& guiContext, uint64_t index) {
  ...
  std::rotate(entry.entries.begin() + index, entry.entries.end(),
              entry.entries.end());
  std::rotate(entry.entryWrapper.begin() + index, entry.entryWrapper.end(),
              entry.entryWrapper.end());
  std::rotate(entry.deleteEntry.begin() + index, entry.deleteEntry.end(),
              entry.deleteEntry.end());
  std::rotate(entry.entryUp.begin() + index, entry.entryUp.end(),
              entry.entryUp.end());
  std::rotate(entry.entryDown.begin() + index, entry.entryDown.end(),
              entry.entryDown.end());
  rotateChild(entry.entryContainer.base, index + 1,
              getChildren(entry.entryContainer.base, guiContext).size(),
              getChildren(entry.entryContainer.base, guiContext).size(),
              guiContext);
...
template <typename ValueEntry>
void _swapIndicesListEntry(ListEntry<ValueEntry>& entry, GUIContext& guiContext,
...
template <typename ValueEntry>
void updateEntry(ListEntry<ValueEntry>& entry, GUIContext& guiContext,
                 Engine& engine, CommandContext& context) {
  ...
  if (clicked) {
    if (pointInElement(cursor, properties(entry.addEntry.base, guiContext))) {
      ...
      for (uint64_t i = 0; i < entry.numEntries; i++) {
        if (pointInElement(cursor,
                           properties(entry.deleteEntry[i].base, guiContext))) {
          _removeIndexFromListEntry(entry, guiContext, i);

          break;
        } else if (pointInElement(
                       cursor, properties(entry.entryUp[i].base, guiContext))) {
          _swapIndicesListEntry(entry, guiContext, i, i - 1);

          break;
        } else if (pointInElement(cursor, properties(entry.entryDown[i].base,
                                                     guiContext))) {
          _swapIndicesListEntry(entry, guiContext, i, i + 1);
...
template <typename ValueEntry>
void submitEntries(std::span<ListEntry<ValueEntry>*> const entries,
                   GUIContext& context) {
...
template <typename ValueEntry>
void dropEntry(ListEntry<ValueEntry>& entry, Engine& engine,
               GUIContext& guiContext) {
...
template <typename ValueEntry>
void loadValue(ListEntry<ValueEntry>& entry,
               typename ListEntry<ValueEntry>::ValueType const& value,
               GUIContext& guiContext) {
...
template <typename T>
ListEntry<UploadEntry<T>>::ValueType getValue(
    ListEntry<UploadEntry<T>> const& entry) {
