...
void _fontTerminate() { FT_Done_FreeType(ftLibrary); }

void _computeSignedDistanceField(const uint8_t* input, uint64_t inputWidth,
                                 uint64_t inputHeight, uint8_t* output,
                                 uint64_t outputWidth, uint64_t outputHeight,
                                 float spreadFactor) {
  ...
  for (uint64_t outY = 0; outY < outputHeight; outY++) {
    for (uint64_t outX = 0; outX < outputWidth; outX++) {
      ...
      //	not completely accurate since we use Euclidean distance, but stop
      // when we find minimum 	Chebyshev distance (I think)
      ...
      while (moves < radius) {
        for (int dx = 0; dx < moves; dx++) {
          if (currentX + direction >= inputWidth || currentX + direction < 0)
            break;

          if (pixelIn !=
              (input[currentX + currentY * inputWidth] > outThreshold)) {
            minimumDistance =
                std::sqrt((currentX - centerX) * (currentX - centerX) +
        ...
        for (int dy = 0; dy < moves; dy++) {
          if (currentY + direction >= inputHeight || currentY + direction < 0)
            break;
      ...
      for (int dy = -radius; dy <= radius; dy++) {
          ...
          for (int dx = -radius; dx <= radius; dx++) {
              ...
              if (pixelPolarity != pixelIn) {
                  ...
                  // float distance = std::max(std::abs(testX - centerX),
      std::abs(testY - centerY));
                  // Compute distance (Euclidean)
                  F32x2 pixelToMidpoint = F32x2(testX - centerX, testY -
      centerY); float distance = pixelToMidpoint.length();

                  // We don't have to check anything else, since we're searching
      circularly minimumDistance = std::min(distance, minimumDistance);
              }
          }
      }
      */
    ...
    foundMinimumDistance:
      ...
      // TODO: better name?
      uint8_t alphaDistance = static_cast<uint8_t>(
          std::clamp(scaledDistance + 0.5f, 0.0f, 1.0f) * 0xff);
...
Font compileSignedDistanceField(const char* inputFontFile, int inputFontSize,
                                const char* outputFile, int outputFieldSize,
                                float spreadFactor) {
  // TODO: shouldn't pad the glpyh, only the field, so we need some extra math
  // in computing the signed distance field on a glyph
  ...
  if (FT_Error error = FT_New_Face(ftLibrary, inputFontFile, 0, &face))
    VIVIUM_LOG(LogSeverity::FATAL, "Failed to load font at {}, error: {}",
               inputFontFile, error);
  ...
  font.imageDimensions =
      I32x2(VIVIUM_CHARACTERS_TO_EXTRACT * fieldPaddedSize, fieldPaddedSize);
  font.fontSpriteSize = I32x2(fieldPaddedSize);
  font.data =
      std::vector<uint8_t>(font.imageDimensions.x * font.imageDimensions.y, 0);

  uint8_t* glyphPixels = reinterpret_cast<uint8_t*>(
      std::malloc(glyphPaddedSize * glyphPaddedSize));
  uint8_t* glyphDistanceField = reinterpret_cast<uint8_t*>(
      std::malloc(fieldPaddedSize * fieldPaddedSize));

  for (uint8_t character = 0; character < VIVIUM_CHARACTERS_TO_EXTRACT;
       character++) {
    // TODO: FT_LOAD_RENDER probably a bad flag
    if (FT_Load_Char(face, character, FT_LOAD_RENDER)) {
      VIVIUM_LOG(LogSeverity::ERROR, "Failed to extract character {} from font",
                 character);
    ...
    // Deal with characters that might be larger than font size including
    // padding
    if (glyphHeight + padding >= glyphPaddedSize)
      glyphHeight = glyphPaddedSize - padding;
    if (glyphWidth + padding >= glyphPaddedSize)
      glyphWidth = glyphPaddedSize - padding;

    for (uint32_t y = halfPadding; y < glyphHeight + halfPadding; y++) {
      for (uint32_t x = halfPadding; x < glyphWidth + halfPadding; x++) {
        glyphPixels[x + y * glyphPaddedSize] =
            face->glyph->bitmap
                .buffer[(x - halfPadding) + (y - halfPadding) * glyphWidth];
      }
    }

    // Compute distance field
    _computeSignedDistanceField(glyphPixels, glyphPaddedSize, glyphPaddedSize,
                                glyphDistanceField, fieldPaddedSize,
                                fieldPaddedSize, spreadFactor);
    ...
    for (uint64_t fieldY = 0; fieldY < fieldPaddedSize; fieldY++) {
      for (uint64_t fieldX = 0; fieldX < fieldPaddedSize; fieldX++) {
        ...
        font.data[characterBufferOffset + fieldX +
                  fieldY * font.imageDimensions.x] = source;
      }
    }

    AtlasIndex index =
        textureAtlasIndex(font.imageDimensions, font.fontSpriteSize, character);

    // Scaling from glyph parameters to field parameters
    float scaleRatio = static_cast<float>(fieldPaddedSize) /
                       static_cast<float>(glyphPaddedSize);
    // Scaling from field parameters to UV coordinates
    F32x2 uvRatio = F32x2(1.0f) / font.imageDimensions;

    I32x2 fieldSize =
        F32x2(glyphWidth, glyphHeight) * scaleRatio + I32x2(halfPadding);
    I32x2 fieldBearing =
        F32x2(face->glyph->bitmap_left, face->glyph->bitmap_top) * scaleRatio;
    int fieldAdvance =
        (static_cast<int>(face->glyph->advance.x * scaleRatio) >> 6) +
        halfPadding;

    F32x2 uvSize = F32x2(fieldSize) * uvRatio;

    font.characters[character] =
        FontCharacter{fieldSize,  fieldBearing,          fieldAdvance,
                      index.left, index.left + uvSize.x, index.top + uvSize.y,
                      index.top};
  }

  // TODO: remove
  stbi_write_png("testGame/res/font.png", font.imageDimensions.x,
                 font.imageDimensions.y, 1, font.data.data(),
                 font.imageDimensions.x);
...
void writeDistanceFieldFont(const char* outputFontFile, Font font) {
  ...
  // Write atlas
  outputFile.write(reinterpret_cast<const char*>(&font.fontSpriteSize),
                   sizeof(I32x2));
  // Write image dimensions
  outputFile.write(reinterpret_cast<const char*>(&font.imageDimensions),
                   sizeof(I32x2));
  // Write character table
  outputFile.write(reinterpret_cast<const char*>(font.characters.data()),
                   font.characters.size() * sizeof(FontCharacter));
  // Write distance field size
  uint64_t fontDataSize = font.data.size();
  outputFile.write(reinterpret_cast<const char*>(&fontDataSize),
                   sizeof(uint64_t));
  // Write distance field
  outputFile.write(reinterpret_cast<const char*>(font.data.data()),
                   font.data.size());
...
Font createFontFile(const char* filename, int fontSize) {
  ...
  font.data = std::vector<uint8_t>(
      VIVIUM_CHARACTERS_TO_EXTRACT * fontSize * fontSize * 4, 0);
  font.imageDimensions =
      I32x2(VIVIUM_CHARACTERS_TO_EXTRACT * fontSize, fontSize);
  ...
  for (uint8_t character = 0; character < VIVIUM_CHARACTERS_TO_EXTRACT;
    ...
    for (uint32_t y = 0; y < glyphHeight; y++) {
      ...
      for (uint32_t x = 0; x < glyphWidth; x++) {
        ...
        uint64_t index =
            y * (VIVIUM_CHARACTERS_TO_EXTRACT * fontSize) + bufferOffset + x;
    ...
    font.characters[character] = FontCharacter{
        ...
        index.top + face->glyph->bitmap.rows /
                        static_cast<float>(font.imageDimensions.y),
        index.top};
  }
...
Font createFontDistanceField(const char* filename) {
  ...
  VIVIUM_ASSERT(magic[0] == 's' && magic[1] == 'd' && magic[2] == 'f',
                "Expected magic at start of distance field file");
  ...
  inputFile.read(reinterpret_cast<char*>(font.characters.data()),
                 font.characters.size() * sizeof(FontCharacter));
