...
namespace Vivium {
BatchSpecification::BatchSpecification(uint64_t vertexCount,
                                       uint64_t indexCount,
...
void submitElementBatch(Batch& batch, uint64_t elementIndex,
  ...
  const BufferLayout::Element& element =
      batch.bufferLayout.elements[elementIndex];
  ...
  for (uint64_t i = 0; i < elementCount; i++) {
    ...
    setBuffer(batch.vertexStaging.resource, vertexIndex,
              data.data() + elementDataIndex * element.size, element.size);
...
void submitRectangleBatch(Batch& batch, uint64_t elementIndex, float left,
  ...
  float data[8] = {left, bottom, right, bottom, right, top, left, top};

  submitElementBatch(
      batch, elementIndex,
      std::span<uint8_t>(reinterpret_cast<uint8_t*>(data), sizeof(data)));
}

void endShapeBatch(Batch& batch, uint64_t vertexCount,
                   const std::span<const uint16_t> indicies) {
  uint16_t* indexMapping = reinterpret_cast<uint16_t*>(
      getBufferMapping(batch.indexStaging.resource));

  for (uint64_t i = 0; i < indicies.size(); i++) {
    indexMapping[batch.indexBufferIndex + i] =
        static_cast<uint16_t>(indicies[i] + batch.verticesSubmitted);
  }
...
Batch submitBatch(ResourceManager& manager, BatchSpecification specification) {
  ...
  submitResource(
      manager, staging.data(), MemoryType::STAGING,
      std::vector<BufferSpecification>(
          {BufferSpecification(
               specification.vertexCount * specification.bufferLayout.stride,
               BufferUsage::STAGING),
           BufferSpecification(specification.indexCount * sizeof(uint16_t),
                               BufferUsage::STAGING)}));

  std::array<BufferReference, 2> device;

  submitResource(
      manager, device.data(), MemoryType::DEVICE,
      std::vector<BufferSpecification>(
          {BufferSpecification(
               specification.vertexCount * specification.bufferLayout.stride,
               BufferUsage::VERTEX),
...
void endSubmissionBatch(Batch& batch, CommandContext& context, Engine& engine) {
  ...
  cmdTransferBuffer(context, batch.vertexStaging.resource,
                    batch.verticesSubmitted * batch.bufferLayout.stride, 0,
                    batch.vertexDevice.resource);
  cmdTransferBuffer(context, batch.indexStaging.resource,
                    batch.indexBufferIndex * sizeof(uint16_t), 0,
                    batch.indexDevice.resource);
