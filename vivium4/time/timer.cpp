...
bool nanosleep(long long nanoseconds) {
  ...
  if (!SetWaitableTimer(m_sleepTimer.waitableTimer, &li, 0, NULL, NULL,
                        FALSE)) {
  ...
  Windows::DWORD ret =
      Windows::WaitForSingleObject(m_sleepTimer.waitableTimer, INFINITE);
...
// NOTE: untested, don't have a linux machine, nor desire to build for linux
// just yet
bool nanosleep(long long nanoseconds) {
  timespec time;
...
float Timer::m_getSecondsBetween(clock_type::time_point start,
...
Timer::Timer() : m_time(std::chrono::steady_clock::now()) {}
...
void Timer::pause() { m_secondsElapsedDuringPause = getTime(); }
...
bool Timer::isPaused() const { return m_secondsElapsedDuringPause != 0.0f; }
}  // namespace Time
}  // namespace Vivium