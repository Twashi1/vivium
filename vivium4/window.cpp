...
void dropWindow(Window& window, Engine& engine) {
  ...
  for (uint32_t i = 0; i < VIVIUM_FRAMES_IN_FLIGHT; i++) {
    vkDestroySemaphore(engine.device, window.renderFinishedSemaphores[i],
                       nullptr);
    vkDestroySemaphore(engine.device, window.imageAvailableSemaphores[i],
                       nullptr);
...
void _createSwapChain(Window& window, Engine& engine) {
  Engine::SwapChainSupportDetails swapChainSupport =
      _querySwapChainSupport(engine.physicalDevice, window.surface);
  ...
  VIVIUM_VK_CHECK(vkCreateSwapchainKHR(engine.device, &createInfo, nullptr,
                                       &window.swapChain),
                  "Failed to create swap chain");

  vkGetSwapchainImagesKHR(engine.device, window.swapChain, &imageCount,
                          nullptr);
  window.swapChainImages.resize(imageCount);
  vkGetSwapchainImagesKHR(engine.device, window.swapChain, &imageCount,
                          window.swapChainImages.data());
...
void _createImageViews(Window& window, Engine& engine) {
  ...
  for (int i = 0; i < window.swapChainImages.size(); i++) {
    ...
    VIVIUM_VK_CHECK(vkCreateImageView(engine.device, &createInfo, nullptr,
...
void _createMultisampleColorImages(Window& window, Engine& engine) {
  if (window.multisampleCount & VK_SAMPLE_COUNT_1_BIT) {
    ...
    VIVIUM_VK_CHECK(vkCreateImage(engine.device, &imageCreateInfo, nullptr,
                                  &window.multisampleColorImage),
                    "Failed to create image");

    VkMemoryRequirements memoryRequirements;
    vkGetImageMemoryRequirements(engine.device, window.multisampleColorImage,
                                 &memoryRequirements);
    ...
    memoryAllocateInfo.memoryTypeIndex =
        _findMemoryType(engine, memoryRequirements.memoryTypeBits,
                        VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT);

    vkAllocateMemory(engine.device, &memoryAllocateInfo, nullptr,
                     &window.multisampleColorMemory);

    // TODO: only 1 image per allocator as is
    vkBindImageMemory(engine.device, window.multisampleColorImage,
                      window.multisampleColorMemory, 0);
    ...
    */
    ...
    VIVIUM_VK_CHECK(vkCreateImageView(engine.device, &viewCreateInfo, nullptr,
...
void _createRenderPass(Window& window, Engine& engine) {
  ...
  renderPassInfo.attachmentCount =
      window.multisampleCount & VK_SAMPLE_COUNT_1_BIT ? 1 : 2;
...
void _createFramebuffers(Window& window, Engine& engine) {
  ...
  for (uint32_t i = 0; i < window.swapChainImages.size(); i++) {
    ...
    VIVIUM_VK_CHECK(
        vkCreateFramebuffer(engine.device, &framebufferInfo, nullptr,
                            &window.swapChainFramebuffers[i]),
...
void _deleteSwapChain(Window& window, Engine& engine) {
  if (!(window.multisampleCount & VK_SAMPLE_COUNT_1_BIT)) {
    ...
    vkDestroyImageView(engine.device, window.multisampleColorImageView,
                       nullptr);
...
void _recreateSwapChain(Window& window, Engine& engine) {
  ...
  VIVIUM_LOG(LogSeverity::DEBUG, "Recreating swapchain with {} {}", width,
             height);
...
VkPresentModeKHR _chooseSwapPresentMode(
  ...
  for (const auto& availablePresentMode : availablePresentModes) {
    // TODO: investigate more present modes, for now we're going to use
    // immediate
    if (availablePresentMode == VK_PRESENT_MODE_IMMEDIATE_KHR) {
      VIVIUM_LOG(LogSeverity::DEBUG,
                 "Using immediate mode, could cause vertical tearing");
...
VkExtent2D _chooseSwapExtent(Window& window,
...
void _setWindowOptions(Window& window, WindowOptions const& options) {
  ...
  glfwSetWindowSize(window.glfwWindow, options.dimensions.x,
                    options.dimensions.y);
  glfwSetWindowTitle(window.glfwWindow, options.title);

  window.multisampleCount =
      static_cast<VkSampleCountFlagBits>(options.multisampleCount);
}

void _createSurface(Window& window, Engine& engine) {
  VIVIUM_VK_CHECK(glfwCreateWindowSurface(engine.instance, window.glfwWindow,
                                          nullptr, &window.surface),
...
void _createVulkanObjects(Window& window, Engine& engine) {
  ...
  // Validate we have support for presenting with the current physical device
  // and queues
  VIVIUM_ASSERT(_checkSurfaceSupport(engine, window.surface),
                "Failed to select physical device and queues");

  window.multisampleCount = static_cast<VkSampleCountFlagBits>(
      getRequestedMultisamples(engine, window.multisampleCount));
...
void _createWindowCommandBuffers(Window& window, Engine& engine) {
  ...
  allocateInfo.commandBufferCount =
      static_cast<uint32_t>(window.commandBuffers.size());
...
void _createSyncObjects(Window& window, Engine& engine) {
  ...
  for (uint32_t i = 0; i < VIVIUM_FRAMES_IN_FLIGHT; i++) {
    ...
    if (vkCreateSemaphore(engine.device, &semaphoreInfo, nullptr,
                          &window.imageAvailableSemaphores[i]) != VK_SUCCESS ||
        vkCreateSemaphore(engine.device, &semaphoreInfo, nullptr,
                          &window.renderFinishedSemaphores[i]) != VK_SUCCESS ||
        vkCreateFence(engine.device, &fenceInfo, nullptr,
                      &window.inFlightFences[i]) != VK_SUCCESS) {
...
Window createWindow(WindowOptions const& options, Engine& engine) {
  ...
  window.glfwWindow = glfwCreateWindow(
      options.dimensions.x, options.dimensions.y, options.title, NULL, NULL);
...
bool windowIsOpen(Window& window, Engine& engine) {
  ...
  if (shouldClose) vkDeviceWaitIdle(engine.device);
...
void windowBeginRender(Window& window) {
  ...
  renderPassInfo.framebuffer =
      window.swapChainFramebuffers[window.currentImageIndex];
  ...
  VkCommandBuffer& commandBuffer =
      window.commandBuffers[window.currentFrameIndex];
...
void windowBeginFrame(Window& window, CommandContext& context, Engine& engine) {
  ...
  VkResult acquireImageResult = vkAcquireNextImageKHR(
      engine.device, window.swapChain, UINT64_MAX,
      window.imageAvailableSemaphores[window.currentFrameIndex], VK_NULL_HANDLE,
  ...
  vkResetFences(engine.device, 1,
                &window.inFlightFences[window.currentFrameIndex]);
  ...
  VIVIUM_VK_CHECK(
      vkBeginCommandBuffer(window.commandBuffers[window.currentFrameIndex],
                           &beginInfo),
...
void windowEndFrame(Window& window, Engine& engine) {
  VIVIUM_VK_CHECK(
      vkEndCommandBuffer(window.commandBuffers[window.currentFrameIndex]),
  ...
  VkSemaphore signalSemaphores[] = {
      window.renderFinishedSemaphores[window.currentFrameIndex]};
  submitInfo.signalSemaphoreCount = 1;
  submitInfo.pSignalSemaphores = signalSemaphores;

  VIVIUM_VK_CHECK(
      vkQueueSubmit(engine.graphicsQueue, 1, &submitInfo,
                    window.inFlightFences[window.currentFrameIndex]),
  ...
  window.currentFrameIndex =
      (window.currentFrameIndex + 1) % VIVIUM_FRAMES_IN_FLIGHT;
}
}  // namespace Vivium
