...
namespace Vivium {
void _populateDebugMessengerInfo(
...
Engine::QueueFamilyIndices _findQueueFamilies(VkPhysicalDevice device) {
  ...
  for (uint32_t i = 0; i < queueFamilyProperties.size(); i++) {
    ...
    if (properties.queueFlags & VK_QUEUE_GRAPHICS_BIT) {
      ...
      // We can pretty safely make the assumption that any graphics queue also
      // supports present
  ...
  VIVIUM_ASSERT(indices.graphicsFamily != UINT32_MAX &&
                    indices.presentFamily != UINT32_MAX &&
                    indices.transferFamily != UINT32_MAX,
                "Couldn't complete queue indices");

  return indices;
}

bool _checkValidationLayerSupport(
    const std::span<const char* const>& validationLayers) {
  uint32_t layerCount;
...
bool _checkSurfaceSupport(Engine& engine, VkSurfaceKHR surface) {
  ...
  VIVIUM_VK_CHECK(vkGetPhysicalDeviceSurfaceSupportKHR(
                      engine.physicalDevice, queueFamilyIndices.presentFamily,
                      surface, &isPresentSupported),
                  "Failed to get physical device surface support details");
  VIVIUM_ASSERT(isPresentSupported,
                "Present not supported for selected physical device");
  ...
  Engine::SwapChainSupportDetails swapChainSupport =
      _querySwapChainSupport(engine.physicalDevice, surface);
...
Engine::SwapChainSupportDetails _querySwapChainSupport(VkPhysicalDevice device,
...
void _pickPhysicalDevice(Engine& engine,
  ...
  for (uint64_t deviceIndex = 0; deviceIndex < deviceCount; deviceIndex++) {
    ...
    switch (properties.deviceType) {
      ...
      case VK_PHYSICAL_DEVICE_TYPE_INTEGRATED_GPU:
        ...
        break;
      default:
        if (!isIntegrated) engine.physicalDevice = device;
        break;
    }
  }

  VIVIUM_ASSERT(engine.physicalDevice != VK_NULL_HANDLE,
  ...
  VIVIUM_LOG(LogSeverity::DEBUG,
             "Using integrated (suboptimal) device with name: {}",
             properties.deviceName);
}

void _createLogicalDevice(Engine& engine,
  ...
  VIVIUM_VK_CHECK(vkCreateDevice(engine.physicalDevice, &createInfo, nullptr,
                                 &engine.device),
                  "Failed to create logical device");

  vkGetDeviceQueue(engine.device, indices.graphicsFamily, 0,
                   &engine.graphicsQueue);
  vkGetDeviceQueue(engine.device, indices.presentFamily, 0,
                   &engine.presentQueue);
  vkGetDeviceQueue(engine.device, indices.transferFamily, 0,
                   &engine.transferQueue);
}

void _createInstance(Engine& engine,
                     const std::span<const char* const> validationLayers,
                     const std::span<const char* const> defaultExtensions) {
  if (Engine::enableValidationLayers &&
      !_checkValidationLayerSupport(validationLayers)) {
    VIVIUM_LOG(LogSeverity::FATAL,
               "Requested validation layers, but some not available");
  }
...
void _setupDebugMessenger(Engine& engine) {
  ...
  auto func = (PFN_vkCreateDebugUtilsMessengerEXT)vkGetInstanceProcAddr(
      engine.instance, "vkCreateDebugUtilsMessengerEXT");
  VkResult returnValue;

  if (func != nullptr) {
    returnValue =
        func(engine.instance, &createInfo, nullptr, &engine.debugMessenger);
  } else {
...
void _checkPerformance(Engine& engine) {
  ...
  if (maxTimeSpent > engine.pollPeriod) {
    float calculatedTimePerFrame =
        engine.pollFramesElapsedTime / engine.pollFramesCounted;
    float calculatedTimePerUpdate =
        engine.pollUpdatesElapsedTime / engine.pollFramesCounted;

    VIVIUM_LOG(LogSeverity::DEBUG, "Max FPS: {}, True FPS: {}, TPF: {}ms",
               1.0f / calculatedTimePerFrame, 1.0f / calculatedTimePerUpdate,
...
void _limitFramerate(Engine& engine) {
  ...
  if (sleep_time > 0.0f) {
    Time::nanosleep(
        static_cast<long long>(static_cast<double>(sleep_time) * 1.0E9));
  }
...
void dropEngine(Engine& engine) {
  ...
  if (Engine::enableValidationLayers) {
    auto func = (PFN_vkDestroyDebugUtilsMessengerEXT)vkGetInstanceProcAddr(
        engine.instance, "vkDestroyDebugUtilsMessengerEXT");
