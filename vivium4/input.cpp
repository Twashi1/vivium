...
void Input::m_updateListener(Listener& listener, int action, float deltaTime) {
  ...
  switch (action) {
    case GLFW_REPEAT:
    case GLFW_PRESS:
      converted = State::PRESS;
      break;
    case GLFW_RELEASE:
      converted = State::RELEASE;
      break;
  }
...
Input::Listener Input::get(Key key) { return m_keyListeners[key]; }

Input::Listener Input::get(Button button) { return m_buttonListeners[button]; }
...
void Input::update(Window& window) {
  ...
  for (int i = _KEY_MIN; i < _KEY_MAX; i++) {
    m_updateListener(m_keyListeners[i], glfwGetKey(window.glfwWindow, i),
                     deltaTime);
  }

  for (int i = _BTN_MIN; i < _BTN_MAX; i++) {
    m_updateListener(m_buttonListeners[i],
                     glfwGetMouseButton(window.glfwWindow, i), deltaTime);
  }
  ...
  if ((m_keyListeners[KEY_LEFT_CONTROL].state & IS_PRESSED_MASK) ||
    ...
    m_currentModifiers =
        static_cast<Modifier>(m_currentModifiers | MOD_CONTROL);
  }
  ...
  if (m_keyListeners[KEY_CAPS_LOCK].state & IS_PRESSED_MASK) {
    m_currentModifiers =
        static_cast<Modifier>(m_currentModifiers | MOD_CAPS_LOCK);
  }

  if (m_keyListeners[KEY_NUM_LOCK].state & IS_PRESSED_MASK) {
    m_currentModifiers =
        static_cast<Modifier>(m_currentModifiers | MOD_NUM_LOCK);
  }
}
...
Input::Modifier Input::getModifiers() { return m_currentModifiers; }

F32x2 Input::getCursor() { return m_cursorPosition; }

Input::CharacterData Input::getCharacters() { return Input::m_frontCharacters; }

Input::Listener::operator bool() const {
  return state & State::IS_PRESSED_MASK;
}

Input::Listener::Listener() : state(State::UP), timeHeld(0.0f) {}
}  // namespace Vivium