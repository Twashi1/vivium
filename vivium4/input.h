...
class Input {
 ...
 public:
  /*! \brief Get key listener data.
   *
   * \param key The key to get data for.
   * \return Listener with key press and duration information.
   */
  static Listener get(Key key);
  /*! \brief Get button listener data.
   *
   * \param key The button to get data for.
   * \return Listener with button press and duration information.
   */
  static Listener get(Button button);

  /*! \brief Update all listeners and keypress information.
   *
   * \param window The window to look for keypreses on.
   */
  static void update(Window& window);

  // TODO: make it so this can be called again on different window
  //  or another function for changing window
  /*! \brief Initialise the input singleton, set to a single window.
   * Initialises all callbacks and listeners.
   * \param window The window to look for keypreses on.
   */
  static void init(Window& window);

  /*! \brief Get all keyboard modifiers pressed last frame.
   *
   * \return Bitset of all modifiers pressed.
   */
  static Modifier getModifiers();

  /*! \brief Get cursor position.
   *
   * Bottom-left corner is 0,0. Top-right corner is screen width, screen height.
   *
   * \return Cursor position as a vector 2D.
   */
  static F32x2 getCursor();

  /*! \brief Get the character codepoints that were pressed last frame.
   *
   * Doesn't automatically deal with special or control characters. Use
   * InputString for additional functionality.
   *
   * \return Get array of codepoints and number of codepoints.
   */
