...
float inertiaPolygon(Polygon const& polygon) {
  ...
  for (uint64_t i = 0; i < polygon.vertices.size(); i++) {
    ...
    inertia += twelth * F32x2::cross(current, next) *
               (F32x2::dot(current, current) + F32x2::dot(next, next) +
                F32x2::dot(current, next));
  }
...
bool containsPolygon(Polygon const& polygon, F32x2 point, Transform transform) {
  ...
  for (uint64_t i = 0; i < polygon.vertices.size(); i++) {
    ...
    // TODO: can simplify this logic probably (need a better way to iterate
    // current and next vertex) Take first non-zero side orientation when we
    // don't have a side orientation
    if (previousSideOrientation == 2 && sideOrientation != 0)
      previousSideOrientation = sideOrientation;
    else if (previousSideOrientation == 0)
      continue;
    else if (previousSideOrientation != sideOrientation)
      return false;
  }
...
Polygon createPolygonVertices(const std::span<const F32x2> vertices) {
  ...
  // TODO: ensure vertices contain origin, requires more of polygon to be
  // constructed to work? VIVIUM_ASSERT(polygon.contains(F32x2(0.0f),
  // Transform::zero()), "Vertices must contain origin");
  ...
  for (F32x2& vertex : polygon.vertices) vertex -= center;
...
Polygon createPolygonBox(F32x2 dimensions) {
  ...
  return createPolygonVertices(
      std::vector<F32x2>({F32x2(left, bottom), F32x2(right, bottom),
                          F32x2(right, top), F32x2(left, top)}));
}
}  // namespace Vivium