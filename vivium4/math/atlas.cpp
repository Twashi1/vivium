...
StitchedAtlasReference submitToStitchedAtlasCreator(
  ...
  StitchedAtlasReference reference =
      submitToStitchedAtlasCreator(image.size, image.data, atlasCreator);
...
StitchedAtlasReference submitToStitchedAtlasCreator(
  ...
  VIVIUM_ASSERT(allocation.data != nullptr,
                "Failed allocation for stitched atlas allocation of size {}",
                allocationSize);
...
StitchedAtlas finishAtlasCreation(StitchedAtlasCreator const& atlasCreator) {
  ...
  VIVIUM_ASSERT(atlas.data != nullptr,
                "Failed allocation for stitched atlas of size {}",
                allocationSize);
  ...
  for (uint64_t i = 0; i < atlasCreator.allocations.size(); i++) {
    ...
    for (uint64_t y = 0; y < allocation.reference.size.y; y++) {
      std::memcpy(atlas.data + atlasRow * y + allocationOffset,
                  allocation.data + allocationRow * y, allocationRow);
    }
  }
...
AtlasIndex convertStitchedAtlasReference(StitchedAtlasReference reference,
  ...
  // TODO: check y math
  return textureAtlasIndex(atlas.size, I32x2(1), reference.size,
...
AtlasIndex _calculateAtlasIndex(int left, int right, int bottom, int top,
  ...
  atlasIndex.scale = F32x2(atlasIndex.right - atlasIndex.left,
                           atlasIndex.bottom - atlasIndex.top);
...
AtlasIndex textureAtlasIndex(I32x2 atlasSize, I32x2 spriteSize, int index) {
  ...
  return _calculateAtlasIndex(xIndex, xIndex + 1, yIndex, yIndex + 1, atlasSize,
                              spriteSize);
}

AtlasIndex textureAtlasIndex(I32x2 atlasSize, I32x2 spriteSize, I32x2 index) {
  return _calculateAtlasIndex(index.x, index.x + 1, index.y, index.y + 1,
                              atlasSize, spriteSize);
}

AtlasIndex textureAtlasIndex(I32x2 atlasSize, I32x2 spriteSize, I32x2 topLeft,
                             I32x2 bottomRight) {
  return _calculateAtlasIndex(topLeft.x, bottomRight.x + 1, bottomRight.y,
                              topLeft.y + 1, atlasSize, spriteSize);
}
}  // namespace Vivium