...
void groupTest() {
  ...
  View<Owned<int>, Owned<float>> view =
      reg.createView<Owned<int>, Owned<float>>();
  ...
  for (ViewElement<Owned<int>, Owned<float>>& element : view) {
    ...
    if (curr % 2 == 1) {
      VIVIUM_ASSERT(curr == 303 || curr == 751,
                    "Invalid entity included in view: {}", curr);
    }
    ...
    VIVIUM_ASSERT(curr == static_cast<int>(element.get<float>()),
                  "Float part not equal, didn't have float part?");
  }
  ...
  for (int i = 0; i < 1000; i++) {
    if (i == 303 || i == 751) {
      ...
      VIVIUM_ASSERT(numbers[i] + (i % 2 == 1) == 1, "Index {} was set invalid",
                    i);
    }
  }
