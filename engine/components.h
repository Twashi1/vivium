...
/*
Perspective perspective = orthogonalPerspective2D(windowDimensions(window),
F32x2(0.0f), 0.0f, 1.0f);

setBuffer(guiContext.button.storageBuffer.resource, 0,
guiContext.button.buttons.data(), guiContext.button.buttons.size() *
sizeof(_GUIButtonInstanceData)); cmdBindPipeline(context,
guiContext.button.pipeline.resource); cmdBindVertexBuffer(context,
guiContext.rectVertexBuffer.resource); cmdBindIndexBuffer(context,
guiContext.rectIndexBuffer.resource); cmdBindDescriptorSet(context,
guiContext.button.descriptorSet.resource, guiContext.button.pipeline.resource);
cmdWritePushConstants(context, &perspective, sizeof(Perspective), 0,
ShaderStage::VERTEX, guiContext.button.pipeline.resource);
...
//  some components take their data from children of the entity they have been
//  placed on

// 2. parse the tree, looking for each entity with pipeline components (or just
// go through registry)
//	resolve all dependencies
//	compile to intermediary
//	interpret intermediary to create vulkan objects

enum class VulkanComponent : uint32_t {
  PIPELINE,
  BUFFER_LAYOUT,
  DESCRIPTOR_LAYOUT,
  SHADER,
...
void loadValue(TextureEntry& entry, TextureComponent const& descriptor,
               GUIContext& guiContext);
...
void serialiseRead(BufferComponent* component, T& store) {
  ...
  VIVIUM_ASSERT(head.component == VulkanComponent::BUFFER,
                "Read incorrect component type");
...
void serialiseRead(ShaderComponent* component, T& store) {
  ...
  VIVIUM_ASSERT(head.component == VulkanComponent::SHADER,
                "Read incorrect component type");
...
void serialiseRead(BufferLayoutComponent* component, T& store) {
  ...
  dispatchSerialiseRead(&head, store);
  // TODO: enum strings for logging
  VIVIUM_ASSERT(head.component == VulkanComponent::BUFFER_LAYOUT,
                "Read incorrect component type");
...
void serialiseRead(DescriptorLayoutComponent* component, T& store) {
  ...
  dispatchSerialiseRead(&head, store);
  // TODO: enum strings for logging
  VIVIUM_ASSERT(head.component == VulkanComponent::DESCRIPTOR_LAYOUT,
                "Read incorrect component type");
...
void serialiseRead(DescriptorSetComponent* component, T& store) {
  ...
  dispatchSerialiseRead(&head, store);
  // TODO: enum strings for logging
  VIVIUM_ASSERT(head.component == VulkanComponent::DESCRIPTOR_SET,
                "Read incorrect component type");
...
void serialiseRead(PipelineComponent* component, T& store) {
  ...
  dispatchSerialiseRead(&head, store);
  // TODO: enum strings for logging
  VIVIUM_ASSERT(head.component == VulkanComponent::PIPELINE,
                "Read incorrect component type");
...
void serialiseRead(TextureComponent* component, T& store) {
  ...
  dispatchSerialiseRead(&head, store);
  // TODO: enum strings for logging
  VIVIUM_ASSERT(head.component == VulkanComponent::TEXTURE,
                "Read incorrect component type");
