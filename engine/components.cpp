...
std::string getString(VulkanComponent component) {
  switch (component) {
    ...
    case VulkanComponent::ENTER_COMPONENT:
...
UniformBindingEntry submitEntry(
    EntrySpecification<UniformBindingEntry> const& spec, GUIContext& guiContext,
    ResourceManager& resourceManager) {
  UniformBindingEntry entry;

  entry.slotEntry =
      submitEntry(EntrySpecification<IntegerTextEntry>("Enter slot"),
                  guiContext, resourceManager);

  entry.stageEntry =
      submitEntry(EntrySpecification<ObjectEntry<ShaderStage>>(
                      ShaderStage::VERTEX,
  ...
  entry.typeEntry =
      submitEntry(EntrySpecification<ObjectEntry<UniformType>>(
                      UniformType::STORAGE_BUFFER,
                      std::vector<UniformType>({UniformType::TEXTURE,
                                                UniformType::STORAGE_BUFFER,
                                                UniformType::UNIFORM_BUFFER})),
                  guiContext, resourceManager);

  entry.entryContainer = createContainer(
      guiContext,
...
void setupEntry(UniformBindingEntry& entry, ResourceManager& manager,
...
void updateEntry(UniformBindingEntry& entry, GUIContext& guiContext,
                 Engine& engine, CommandContext& context) {
...
void submitEntries(std::span<UniformBindingEntry*> const entries,
                   GUIContext& guiContext) {
...
void dropEntry(UniformBindingEntry& entry, Engine& engine,
               GUIContext& guiContext) {
...
void loadValue(UniformBindingEntry& entry, UniformBinding const& binding,
               GUIContext& guiContext) {
...
BufferLayoutEntry submitEntry(EntrySpecification<BufferLayoutEntry> const& spec,
                              GUIContext& guiContext,
                              ResourceManager& resourceManager) {
  ...
  objectEntrySpec.options = {ShaderDataType::VEC2, ShaderDataType::VEC3,
                             ShaderDataType::FLOAT, ShaderDataType::INT};
...
void setupEntry(BufferLayoutEntry& entry, ResourceManager& manager,
                Engine& engine, CommandContext& context,
                GUIContext& guiContext) {
  setupEntry(entry.typesEntry, manager, engine, context, guiContext);
}

void updateEntry(BufferLayoutEntry& entry, GUIContext& guiContext,
                 Engine& engine, CommandContext& context) {
  updateEntry(entry.typesEntry, guiContext, engine, context);
}

void submitEntries(std::span<BufferLayoutEntry*> const entries,
                   GUIContext& guiContext) {
  for (BufferLayoutEntry* entry : entries) {
    ListEntry<ObjectEntry<ShaderDataType>>* listEntries[] = {
        &entry->typesEntry};
...
void dropEntry(BufferLayoutEntry& entry, Engine& engine,
               GUIContext& guiContext) {
  dropEntry(entry.typesEntry, engine, guiContext);
}

void loadValue(BufferLayoutEntry& entry, BufferLayoutComponent const& layout,
               GUIContext& guiContext) {
...
ShaderComponent getValue(ShaderEntry const& entry) {
  ...
  // std::string::iterator dot = std::find(shaderFilename.begin(),
  // shaderFilename.end(), '.');
...
ShaderEntry submitEntry(EntrySpecification<ShaderEntry> const& spec,
                        GUIContext& guiContext,
                        ResourceManager& resourceManager) {
  ShaderEntry entry;

  entry.container = createContainer(
      guiContext,
      ContainerSpecification(nullGUIParent(), ContainerOrdering::VERTICAL,
                             OffsetMethod::EXTENT));
  ...
  entry.filenameEntry =
      submitEntry(EntrySpecification<StringTextEntry>("Enter filename"),
                  guiContext, resourceManager);
  entry.stageEntry = submitEntry(
      EntrySpecification<ObjectEntry<ShaderStage>>(
          ShaderStage::VERTEX, {ShaderStage::FRAGMENT, ShaderStage::VERTEX}),
      guiContext, resourceManager);
...
void setupEntry(ShaderEntry& entry, ResourceManager& manager, Engine& engine,
...
void updateEntry(ShaderEntry& entry, GUIContext& guiContext, Engine& engine,
                 CommandContext& context) {
  updateEntry(entry.filenameEntry, guiContext, engine, context);
  updateEntry(entry.stageEntry, guiContext, engine, context);
}

void submitEntries(std::span<ShaderEntry*> const entries,
                   GUIContext& guiContext) {
...
void loadValue(ShaderEntry& entry, ShaderComponent const& shader,
               GUIContext& guiContext) {
...
DescriptorLayoutEntry submitEntry(
    EntrySpecification<DescriptorLayoutEntry> const& spec,
    GUIContext& guiContext, ResourceManager& resourceManager) {
  DescriptorLayoutEntry entry;

  entry.entrySpec = new EntrySpecification<UniformBindingEntry>();
  entry.bindingEntries = submitEntry(
      EntrySpecification<ListEntry<UniformBindingEntry>>(5, entry.entrySpec),
      guiContext, resourceManager);
...
void setupEntry(DescriptorLayoutEntry& entry, ResourceManager& manager,
                Engine& engine, CommandContext& context,
                GUIContext& guiContext) {
  setupEntry(entry.bindingEntries, manager, engine, context, guiContext);
}

void updateEntry(DescriptorLayoutEntry& entry, GUIContext& guiContext,
                 Engine& engine, CommandContext& context) {
  updateEntry(entry.bindingEntries, guiContext, engine, context);
}

void submitEntries(std::span<DescriptorLayoutEntry*> const entries,
                   GUIContext& guiContext) {
...
void dropEntry(DescriptorLayoutEntry& entry, Engine& engine,
               GUIContext& guiContext) {
...
void loadValue(DescriptorLayoutEntry& entry,
               DescriptorLayoutComponent const& descriptorLayout,
               GUIContext& guiContext) {
...
BufferEntry submitEntry(EntrySpecification<BufferEntry> const& spec,
                        GUIContext& guiContext,
                        ResourceManager& resourceManager) {
  BufferEntry entry;
  ...
  entry.usage =
      submitEntry(EntrySpecification<ObjectEntry<BufferUsage>>(
                      BufferUsage::VERTEX,
  ...
  entry.data = submitEntry(
      EntrySpecification<ListEntry<FloatTextEntry>>(16, entry.entrySpec),
      guiContext, resourceManager);
...
void setupEntry(BufferEntry& entry, ResourceManager& manager, Engine& engine,
                CommandContext& context, GUIContext& guiContext) {
...
void updateEntry(BufferEntry& entry, GUIContext& guiContext, Engine& engine,
                 CommandContext& context) {
  updateEntry(entry.data, guiContext, engine, context);
  updateEntry(entry.usage, guiContext, engine, context);
}

void submitEntries(std::span<BufferEntry*> const entries,
                   GUIContext& guiContext) {
  for (BufferEntry* entry : entries) {
    ...
    debugRect(propsUsage.minExtent, propsUsage.maxExtent - propsUsage.minExtent,
              Color(0.0f, 1.0f, 0.0f), guiContext);

    GUIProperties& propsData = properties(entry->data.base, guiContext);
    debugRect(propsData.minExtent, propsData.maxExtent - propsData.minExtent,
              Color(1.0f, 0.0f, 1.0f), guiContext);
  }
}
...
void loadValue(BufferEntry& entry, BufferComponent const& buffer,
               GUIContext& guiContext) {
  loadValue(entry.usage, buffer.usage, guiContext);

  VIVIUM_ASSERT(
      buffer.data.size() % 4 == 0,
      "Buffer data was not multiple of 4, incorrectly assumed float?");

  std::vector<float> bufferDataConverted(buffer.data.size() / 4);
  memcpy(bufferDataConverted.data(), buffer.data.data(),
         bufferDataConverted.size() * sizeof(float));
...
PipelineEntry submitEntry(EntrySpecification<PipelineEntry> const& spec,
                          GUIContext& guiContext,
                          ResourceManager& resourceManager) {
  PipelineEntry entry;
  ...
  entry.bufferLayout = submitEntry(EntrySpecification<UploadEntry<Entity>>(
                                       "Buffer layout", spec.heldItemPointer),
                                   guiContext, resourceManager);
  entry.descriptorLayout =
      submitEntry(EntrySpecification<UploadEntry<Entity>>("Descriptor layout",
                                                          spec.heldItemPointer),
                  guiContext, resourceManager);
  entry.vertexShader = submitEntry(EntrySpecification<UploadEntry<Entity>>(
                                       "Vertex shader", spec.heldItemPointer),
                                   guiContext, resourceManager);
  entry.fragmentShader =
      submitEntry(EntrySpecification<UploadEntry<Entity>>("Fragment shader",
                                                          spec.heldItemPointer),
                  guiContext, resourceManager);
  entry.vertexBuffer = submitEntry(EntrySpecification<UploadEntry<Entity>>(
                                       "Vertex buffer", spec.heldItemPointer),
                                   guiContext, resourceManager);
  entry.indexBuffer = submitEntry(EntrySpecification<UploadEntry<Entity>>(
                                      "Index buffer", spec.heldItemPointer),
                                  guiContext, resourceManager);
  entry.descriptorSet = submitEntry(EntrySpecification<UploadEntry<Entity>>(
                                        "Descriptor set", spec.heldItemPointer),
                                    guiContext, resourceManager);
...
void setupEntry(PipelineEntry& entry, ResourceManager& manager, Engine& engine,
                CommandContext& context, GUIContext& guiContext) {
...
void updateEntry(PipelineEntry& entry, GUIContext& guiContext, Engine& engine,
                 CommandContext& context) {
...
void submitEntries(std::span<PipelineEntry*> const entries,
                   GUIContext& guiContext) {
  for (PipelineEntry* entry : entries) {
    UploadEntry<Entity>* uploads[] = {
        &entry->bufferLayout,   &entry->descriptorLayout, &entry->vertexShader,
        &entry->fragmentShader, &entry->vertexBuffer,     &entry->indexBuffer,
        &entry->descriptorSet};
...
void loadValue(PipelineEntry& entry, PipelineComponent const& pipeline,
               GUIContext& guiContext) {
  ...
  for (Entity entity : entities) {
    if (entry.registry->hasComponent<BufferComponent>(entity)) {
      ...
      VIVIUM_LOG(LogSeverity::ERROR,
                 "Passed entity {} to descriptor set, but had no "
                 "compatible/implemented type",
                 entity);
    }
...
DescriptorSetEntry submitEntry(
    EntrySpecification<DescriptorSetEntry> const& spec, GUIContext& guiContext,
    ResourceManager& resourceManager) {
  ...
  entry.uniformData = submitEntry(
      EntrySpecification<ListEntry<UploadEntry<Entity>>>(4, entry.entrySpec),
      guiContext, resourceManager);
...
void setupEntry(DescriptorSetEntry& entry, ResourceManager& manager,
                Engine& engine, CommandContext& context,
                GUIContext& guiContext) {
  setupEntry(entry.uniformData, manager, engine, context, guiContext);
}

void updateEntry(DescriptorSetEntry& entry, GUIContext& guiContext,
                 Engine& engine, CommandContext& context) {
  updateEntry(entry.uniformData, guiContext, engine, context);
}

void submitEntries(std::span<DescriptorSetEntry*> const entries,
                   GUIContext& guiContext) {
...
void dropEntry(DescriptorSetEntry& entry, Engine& engine,
               GUIContext& guiContext) {
  delete entry.entrySpec;
...
void loadValue(DescriptorSetEntry& entry,
               DescriptorSetComponent const& descriptor,
               GUIContext& guiContext) {
...
TextureEntry submitEntry(EntrySpecification<TextureEntry> const& spec,
                         GUIContext& guiContext,
                         ResourceManager& resourceManager) {
  TextureEntry entry;
  ...
  entry.filename =
      submitEntry(EntrySpecification<StringTextEntry>("Enter filename"),
                  guiContext, resourceManager);
  entry.filter =
      submitEntry(EntrySpecification<ObjectEntry<TextureFilter>>(
                      TextureFilter::LINEAR,
                      std::vector<TextureFilter>(
                          {TextureFilter::LINEAR, TextureFilter::NEAREST})),
                  guiContext, resourceManager);
  entry.format =
      submitEntry(EntrySpecification<ObjectEntry<TextureFormat>>(
                      TextureFormat::RGBA,
                      std::vector<TextureFormat>(
                          {TextureFormat::RGBA, TextureFormat::MONOCHROME})),
                  guiContext, resourceManager);
...
void setupEntry(TextureEntry& entry, ResourceManager& manager, Engine& engine,
                CommandContext& context, GUIContext& guiContext) {
...
void updateEntry(TextureEntry& entry, GUIContext& guiContext, Engine& engine,
                 CommandContext& context) {
...
void submitEntries(std::span<TextureEntry*> const entries,
                   GUIContext& guiContext) {
...
void loadValue(TextureEntry& entry, TextureComponent const& component,
               GUIContext& guiContext) {
